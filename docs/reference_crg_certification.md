...
| **INTEG-ECRITURE-01** | **UN IMPORT N'EST ANNULABLE QUE SI TOUTE ÉCRITURE PASSE PAR UN SEUL ENDROIT QUI LA NOTE.** Le moteur historique ne revenait que sur `biens` et `immeubles` ; propriétaires, tiers, mandats, baux et statuts de locataires étaient « sautés » et restaient. Dès qu'une écriture s'échappe — une table oubliée, un `INSERT` direct « juste pour cette fois » — l'annulation ment, et elle ment SILENCIEUSEMENT : elle rapporte un succès sur ce qu'elle a su défaire. `crgi_ecrire()` est donc le seul point d'écriture ; il journalise table, identifiant, action et **l'état d'avant sur les seules colonnes modifiées** — journaliser la ligne entière ferait revenir, à l'annulation, des champs qu'un humain a changés depuis. `crgi_defaire()` rejoue à l'envers : création supprimée, modification restaurée, **ligne jamais touchée jamais approchée**. Un échec de suppression est NOMMÉ, jamais avalé. | ÉPROUVÉE |
| **INTEG-SIMULATION-01** | **UNE SIMULATION DOIT COMPTER CE QU'UN IMPORT FERAIT, PAS CE QU'ELLE-MÊME ARRIVE À FAIRE.** Ne rendant aucun identifiant pour une création, la simulation laissait la carte des liens vide : les baux ne trouvaient plus leur bien et se comptaient « ignorés » **598 fois**, les rôles ne se créaient **jamais** (9 au lieu de 1 757). Le mode d'essai avait sa propre défaillance et l'attribuait aux données — la pire forme de faux négatif : on cherche le défaut dans le corpus, il est dans l'instrument. Toute simulation rend donc les mêmes valeurs de retour qu'une exécution réelle, identifiants compris, sous forme **négative** reconnaissable ; et une garde refuse qu'un identifiant fictif atteigne une écriture réelle. | ÉPROUVÉE |
| **INTEG-RAPPROCHEMENT-01** | **UN RAPPROCHEMENT QUI NE RAPPROCHE RIEN NE SE VOIT PAS.** La recherche d'un tiers comparait un nom normalisé — majuscules, accents et ponctuation retirés — à un `UPPER(TRIM())` qui conserve points et apostrophes : `M. ET MME X` ne rejoignait jamais `M ET MME X`. Aucune exception, aucun ralentissement, aucun journal d'erreur — **seulement des doublons créés en silence**, visibles uniquement en comptant les objets (1 808 créations sur une base de 961 tiers). **Les deux membres d'une comparaison d'identité passent par la MÊME fonction de normalisation**, appelée explicitement de part et d'autre — jamais une normalisation SQL d'un côté et PHP de l'autre. Second piège au même endroit : concaténer `raison_sociale` ET `nom`, qui portent la même valeur pour une personne morale, donne « SCI FAVRESCI FAVRE ». | ÉPROUVÉE |
| **INTEG-V2-REFERENTIEL** | **UNE VARIANTE DE LECTEUR N'EST PAS UN RÉFÉRENTIEL D'IDENTIFIANTS.** Le moteur ICS a deux variantes, `lyon` et `emery_immo`, qui lisent le MÊME référentiel de codes : porter le format dans la clé d'objet faisait entrer **22 immeubles et 31 lots** deux fois. Le référentiel suit l'entité qui frappe le code — l'agence — jamais le lecteur qui l'a déchiffré, et il se déduit du corpus par majorité pour qu'un changement d'éditeur soit visible au lieu d'être présumé. Corollaire de nommage : le staging porte le nom court de la variante (`lyon`), la base le nom du référentiel (`loca_immo_lyon`) — sans table de correspondance, chaque code est refusé. | ÉPROUVÉE |
| **INTEG-V2-ADRESSE** | **UNE RÈGLE QUI DÉCRIT CE QU'UNE DONNÉE DOIT ÊTRE ÉCARTE LES CAS RARES ; UNE RÈGLE QUI NOMME LE BRUIT N'ÉCARTE QUE LE BRUIT.** Exiger qu'une adresse commence par un numéro ou un type de voie rejetait `Chabrepine`, `LIEUDIT LE MONINSABLE`, `ZAC DU TISSOT` — des adresses valides. Nommer les en-têtes du document (`SITUATION DES LOCATAIRES`, `RECAPITULATIF DES OPERATIONS`) et les raisons sociales des agences n'écarte que les 13 vrais en-têtes et le nom d'agence, et fait passer les adresses renseignées de **512 à 558**. ⚠️ Le filtre s'applique AVANT que la valeur ne serve de clé de repliement : deux immeubles partageant le même en-tête auraient fusionné. Et l'adresse de l'agence n'est jamais celle de l'immeuble — le CRG imprime les deux sur la même page. | ÉPROUVÉE |
| **INTEG-V2-CLE-STOCKEE** | **UNE CLÉ DE RAPPROCHEMENT SE CALCULE SUR LA FORME STOCKÉE, PAS SUR LA FORME LUE.** La base ne garde pas « Monsieur XERRI Florent » mais `nom=XERRI` + `prenoms=Florent` : une clé prise sur la chaîne imprimée ne peut jamais être reconstruite en relisant la base, et la personne serait recréée à chaque dépôt. Normaliser ne suffit pas — il faut normaliser la même REPRÉSENTATION des deux côtés. La fonction vit donc dans **un seul fichier**, requis par le lecteur et par l'écrivain : deux copies sont deux normalisations qui vont diverger. | ÉPROUVÉE |
| **INTEG-V2-RANG** | **LE RANG DANS UNE LISTE EST UNE PROPRIÉTÉ DE LA LISTE, PAS DE L'OBJET.** Poser « le premier code de la liste » comme code principal a fait refuser **trois dépôts sur quatre** : deux groupes du plan se rejoignent parfois sur le même immeuble — repliés après coup par leur adresse — et chacun posait le sien. L'index UNIQUE avait raison. L'unicité d'un attribut distingué se vérifie sur ce qui est DÉJÀ POSÉ, dans la base et dans la passe en cours, jamais sur l'indice d'itération. | ÉPROUVÉE |
| **INTEG-V2-SCHEMA-PARTAGE** | **SUR UNE BASE PARTAGÉE, UN SCHÉMA LU EST UNE PHOTOGRAPHIE, PAS UN CONTRAT.** Une autre session a appliqué une migration entre ma lecture du schéma (22:42) et mon écriture (22:59) — celle-là même dont les contraintes ont fait échouer l'import. Le même phénomène s'était produit une heure plus tôt. Ce n'est pas une erreur de lecture : c'est une hypothèse d'exclusivité jamais formulée. D'où un **registre des migrations**, pour qu'une base dise où elle en est sans qu'on l'ausculte ; et une **transaction par passe**, pour qu'un schéma qui a bougé fasse échouer l'ensemble au lieu de laisser la base à moitié écrite. | ÉPROUVÉE |
| **INTEG-V2-CHIFFRE-DOCTRINE** | **UN CHIFFRE ÉCRIT DANS LA DOCTRINE SE REMESURE AVANT DE SERVIR DE SPÉCIFICATION.** Le schéma V2 justifiait sa table d'occupants multiples par « 121 noms sur 1 268 portent deux personnes ». Remesuré : **88 des 90 noms à barres obliques sont « NOM/// Prénom », une seule personne** ; il y a **2** colocations réelles. Le 121 comptait les séparateurs. Un nombre consigné devient une cible — et s'il a été obtenu en comptant un proxy, on ajuste le code sur le proxy. | ÉPROUVÉE |
...
| **INTEG-P3-PORTEE-HONORAIRES** | **LES HONORAIRES SE COMPTENT SUR LE COMPTE RENDU, DONC ILS NE DISENT RIEN D UN LOT.** Un mandant de vingt lots dont UN est vendu facture toujours ses honoraires sur les dix-neuf autres : en tirer « le bien est vide » pour le vingtieme, c est faire dire au document ce qu il n enonce pas. L argument ne vaut donc QUE si le silence couvre le MANDANT ENTIER — la, plus rien n est appele nulle part, et la facturation prouve que la regie gere encore un portefeuille vide. **Et pour un immeuble ou un lot isole, c est VENDU dans 90 % des cas** : Emmanuel, 08/09/2026, « si un autre locataire n est pas actif sur le meme bien, alors c est effectivement soit vendu (90 % des cas) soit vacant ».  — proposer le cas rare oblige a corriger neuf fois sur dix. Effet mesure : 82 VENDU contre 1 VACANT, la ou la portee mal appliquee en donnait 68 VACANT. | EPROUVEE |
