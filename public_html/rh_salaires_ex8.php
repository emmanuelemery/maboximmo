...
<div id="dlb-modal" style="display:none;position:fixed;inset:0;background:rgba(15,23,42,.55);z-index:9999;align-items:center;justify-content:center;padding:14px;" onclick="if(event.target===this)fermerTelechargementBulletins()">
    ...
    <div id="dlb-boite" style="background:#fff;border-radius:14px;max-width:660px;width:100%;max-height:90vh;display:flex;flex-direction:column;box-shadow:0 20px 60px rgba(0,0,0,.4);overflow:hidden;transition:max-width .15s;">
        ...
        <div style="padding:16px 20px;overflow:auto;flex:1;">
            <div style="font-size:11px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.05em;margin-bottom:6px;">Société</div>
            ...
            <div style="font-size:11px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.05em;margin-bottom:6px;">Mois</div>
            <div style="display:flex;flex-wrap:wrap;gap:7px;margin-bottom:14px;">
                <?php for ($m = 1; $m <= 12; $m++): ?>
                <button type="button" class="filter-pill" data-champ="mois" data-val="<?=$m?>"><?=$dlbMoisCourt[$m]?></button>
                <?php endfor; ?>
            </div>

            <div style="font-size:11px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.05em;margin-bottom:6px;">Année</div>
            <div style="display:flex;flex-wrap:wrap;gap:7px;">
                <?php foreach ($dlbAnnees as $a): ?>
                <button type="button" class="filter-pill" data-champ="annee" data-val="<?=$a?>"><?=$a?></button>
                <?php endforeach; ?>
            </div>

            <div style="font-size:11px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.05em;margin:14px 0 6px;">Contenu</div>
            <div style="display:flex;flex-wrap:wrap;gap:7px;">
                <button type="button" class="filter-pill dlb-large" data-champ="mode" data-val="0">Un PDF par agence</button>
                <button type="button" class="filter-pill dlb-large" data-champ="mode" data-val="1">Un PDF par salarié</button>
            </div>
            ...
            <?php /* Aperçu du découpage. À droite, le PDF RÉELLEMENT produit pour
                     ...
                     qui partira. */ ?>
            <div id="dlb-zone" style="display:none;margin-top:14px;gap:14px;">
                <div id="dlb-apercu" style="flex:1;min-width:0;font-size:13px;color:#334155;"></div>
                <div id="dlb-visu" style="display:none;width:46%;min-width:300px;border:1px solid #e2e8f0;border-radius:10px;overflow:hidden;background:#f8fafc;">
                    ...
                    <iframe id="dlb-visu-frame" style="width:100%;height:52vh;border:none;background:#fff;"></iframe>
                </div>
            </div>
        ...
        <div style="padding:12px 20px;border-top:1px solid #e5e7eb;display:flex;gap:8px;justify-content:flex-end;align-items:center;">
            ...
            <button type="button" class="v2-btn" id="dlb-classer" style="display:none;background:#0f766e;color:#fff;border-color:#0f766e;"
                    onclick="classerDecoupe()">✅ Classer dans les dossiers RH</button>
...
(function(){
  ...
  window.fermerVisu = function(){
    ...
    el('dlb-boite').style.maxWidth = el('dlb-apercu').innerHTML ? '860px' : '660px';
  };
  ...
  window.voirBulletin = function(depotId, idUser, nom){
    ...
    el('dlb-boite').style.maxWidth = '1180px';
    ...
    fetch(url, {credentials:'same-origin', method:'HEAD'})
      .then(r => {
        ...
        el('dlb-visu-ctl').innerHTML = ok
          ...
          : '<span style="color:#b91c1c;font-weight:700;">⛔ Envoi bloqué</span> <span style="color:#b91c1c;">— ' + esc(motif) + '</span>';
      })
  ...
  window.fermerComposeur = function(){
    ...
    el('dlb-boite').style.maxWidth = '860px';
  ...
  function peindre(){
    ...
    el('dlb-resume').innerHTML =
      'Bulletins de <b>' + (bs ? bs.textContent.trim() : '—') + '</b> · <b>'
      + (MOIS[choix.mois-1] || '?') + ' ' + choix.annee + '</b>'
      + '<div style="color:#64748b;margin-top:3px;">'
      + (choix.mode
          ? 'Chaque PDF d’agence est découpé en un bulletin par salarié. Un aperçu est présenté avant tout classement.'
          : 'Un PDF par agence, rassemblés dans un seul ZIP.')
      + '</div>';
  }
  ...
  function analyser(){
    ...
    fetch(URL_DECOUPE + '?mode=preview&' + qs(), {credentials:'same-origin'})
      .then(r => r.json())
      .then(d => {
        b.disabled = false;
        if (!d.ok) {
  ...
  window.classerDecoupe = function(){
    if (!confirm("Classer ces bulletins dans le dossier RH de chaque salarié ?\n\nLes pages non attribuées ne seront pas classées.")) return;
    ...
    fetch(URL_DECOUPE, {method:'POST', body:fd, credentials:'same-origin'})
      .then(r => r.json())
      .then(d => {
        b.disabled = false; b.textContent = '✅ Classer dans les dossiers RH';
        if (!d.ok) { alert('Erreur : ' + (d.error||'inconnue')); return; }
        alert(d.message + ((d.erreurs||[]).length ? '\n\nAnomalies :\n- ' + d.erreurs.join('\n- ') : ''));
      })
      .catch(e => { b.disabled = false; b.textContent = '✅ Classer dans les dossiers RH'; alert('Erreur : ' + e.message); });
  };
