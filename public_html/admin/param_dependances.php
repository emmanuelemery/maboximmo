...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'toggle_lien') {
        ...
        if ($idTB > 0 && $idDep > 0) {
            ...
            $stmtTB = $pdo->prepare("SELECT COUNT(*) FROM societe_types_bien WHERE id = ? AND id_societe = ?");
            $stmtTB->execute([$idTB, $societeId]);
            $okTB = (int)$stmtTB->fetchColumn();
            $stmtDep = $pdo->prepare("SELECT COUNT(*) FROM societe_dependances_exterieurs WHERE id = ? AND id_societe = ?");
            $stmtDep->execute([$idDep, $societeId]);
            $okDep = (int)$stmtDep->fetchColumn();
            if ($okTB && $okDep) {
                $stmtEx = $pdo->prepare("SELECT COUNT(*) FROM societe_type_bien_dependance_ext WHERE id_societe_type_bien = ? AND id_societe_dependance_exterieur = ?");
                $stmtEx->execute([$idTB, $idDep]);
