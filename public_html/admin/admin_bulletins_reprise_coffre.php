...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'reprendre') {
    ...
    foreach ($lignes as $d) {
        ...
        /* La pièce reprise date de son dépôt d'ORIGINE, pas d'aujourd'hui : sans
           cette date, une reprise lancée après coup passerait devant un bulletin
           corrigé déjà classé et le renverrait au rang de version archivée. */
        $r = rhbc_classer_fichier($pdo, $abs, $salarie, $mois, $annee, $tousSal, [
            'via'         => 'reprise',
            'source_date' => (string)($d['upload_date'] ?? ''),
            'origine'     => 'manuel',
            'id_societe'  => $idSoc,
            'id_agence'   => $idAgence,
        ...
        /* « anterieur » = le coffre détient déjà mieux (un corrigé postérieur) : rien
           n'a été écrasé, et la ligne d'origine peut être archivée sans rien perdre. */
        $rapport[] = ['id' => $d['id'], 'nom' => $nom,
                      'etat' => (($r['statut'] ?? '') === 'anterieur'
                                    ? 'déjà au coffre en version plus récente (v' . $r['version'] . ')'
                                    : 'repris au coffre (v' . $r['version'] . ')')
                                . ' — ligne archivée'];
    }
