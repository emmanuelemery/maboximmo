...
// ── Actions rapides ───────────────────────────────────────────────────────────
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_contrat']) && $role_id === 1) {
    verify_csrf_any();
    $pdo->prepare("DELETE FROM agency_contrat WHERE id=?")->execute([(int)$_POST['delete_contrat']]);
    ...
    ?>
      <tr>
        ...
        <td style="text-align:right;white-space:nowrap">
          ...
          <?php if ($role_id===1): ?>
          <form method="post" style="display:inline" onsubmit="return confirm('Supprimer ce contrat ?')"><input type="hidden" name="delete_contrat" value="<?= $c['id'] ?>"><input type="hidden" name="csrf_token" value="<?= h(csrf_token()) ?>"><button type="submit" class="btn-icon danger">
