...
// Immeubles CRG (code_crg renseigné) dont le CP manque mais dont adresse_1 contient un CP à 5 chiffres.
$rows = $pdo->query("
    SELECT id, nom_immeuble, adresse_1, code_postal, ville
    FROM immeubles
    WHERE COALESCE(code_crg,'') <> ''
      AND COALESCE(code_postal,'') = ''
      AND adresse_1 REGEXP '[0-9]{5}'
    ORDER BY id
...
$upd = $pdo->prepare("UPDATE immeubles SET adresse_1=?, code_postal=?, ville=COALESCE(NULLIF(ville,''),?) WHERE id=?");
$n = 0;
foreach ($rows as $r) {
    ...
    $act = '—';
    if ($doable) {
        if ($apply) { $upd->execute([$voie ?: $r['adresse_1'], $cp, $ville ?: null, (int)$r['id']]); $act = '<span class="ok">écrit</span>'; $n++; }
