...
<div id="autosave-indicator" class="autosave-indicator">⏳ Sauvegarde…</div>
<?php if ($saved): ?>
        ...
        <div class="tab-panel <?= $activeTab==='docs' ? 'active':'' ?>" id="tab-docs">
          <div class="form-section">
            <div class="form-section-title">
              📄 Mes documents
              <a href="./rh_documents_user.php?user_id=<?= (int)$userId ?>" class="btn-add-doc" style="float:right;padding:4px 14px;background:#4878a6;color:#fff;border-radius:6px;font-size:11px;font-weight:600;text-decoration:none;font-family:'Sora',sans-serif">+ Ajouter un document</a>
            </div>
...
<script>
// ── Auto-save (post-layout) ──────────────────────────────────────
(function() {
    const indicator = document.getElementById('autosave-indicator');
    const wrap = document.getElementById('profil-form-wrap');
    if (!wrap) { console.error('[AUTO-SAVE] #profil-form-wrap introuvable'); return; }
    let pendingFields = {};
    let saveTimer = null;
    let hideTimer = null;
    ...
    function setIndicator(state, msg) {
        ...
        if (state === 'saved' || state === 'error') {
            hideTimer = setTimeout(function() { indicator.className = 'autosave-indicator'; }, 2500);
        }
    }

    async function flush() {
        if (!Object.keys(pendingFields).length) return;
        var toSave = Object.assign({}, pendingFields);
        pendingFields = {};
        setIndicator('saving', '⏳ Sauvegarde…');
        try {
            var r = await fetch('./api/rh_profil_save_field.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': CSRF_TOKEN },
                body: JSON.stringify({ user_id: PROFILE_USER_ID, fields: toSave, csrf_token: CSRF_TOKEN })
            });
            if (!r.ok) { var txt = await r.text(); throw new Error('HTTP ' + r.status + ': ' + txt.substring(0, 300)); }
            var d = await r.json();
            if (!d.success) throw new Error(d.error || d.message || 'Erreur');
            setIndicator('saved', '✓ Validé');
            console.log('[AUTO-SAVE] OK:', toSave);
        } catch(e) {
            setIndicator('error', '✗ ' + (e.message || 'Erreur'));
            console.error('[AUTO-SAVE] ÉCHEC:', e.message, 'champs:', JSON.stringify(toSave));
            if (typeof showToast === 'function') showToast('Erreur sauvegarde: ' + (e.message || 'Erreur'), 'err');
            Object.assign(pendingFields, toSave);
        }
    }
    ...
    function onFieldChange(e) {
        ...
        var name = el.name;
        if (!name) return;
        pendingFields[name] = el.value;
        schedSave();
    }

    wrap.addEventListener('input',  onFieldChange);
    wrap.addEventListener('change', onFieldChange);
    console.log('[AUTO-SAVE] Listeners attachés sur #profil-form-wrap');

    window.addEventListener('beforeunload', function() {
        if (Object.keys(pendingFields).length) {
            var xhr = new XMLHttpRequest();
            xhr.open('POST', './api/rh_profil_save_field.php', false);
            xhr.setRequestHeader('Content-Type', 'application/json');
            xhr.setRequestHeader('X-CSRF-Token', CSRF_TOKEN);
            xhr.send(JSON.stringify({ user_id: PROFILE_USER_ID, fields: Object.assign({}, pendingFields), csrf_token: CSRF_TOKEN }));
        }
    });
})();
</script>
