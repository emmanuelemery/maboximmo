...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Logo MBI aligné à gauche, occupant ~80% de la hauteur du bandeau. */
    .hero-logo {
      position: absolute; top: 10%; bottom: 10%; left: 28px;
      display: inline-flex; align-items: center;
      background: #fff; border-radius: 16px; padding: 10px 18px;
      box-shadow: 0 10px 26px -12px rgba(0,0,0,.45);
      transition: transform .12s, box-shadow .12s;
    }
    .hero-logo:hover { transform: translateY(-1px); box-shadow: 0 14px 30px -12px rgba(0,0,0,.5); }
    .hero-logo img { height: 100%; width: auto; display: block; }
    @media (max-width: 720px) {
      .hero { padding-top: 150px; }
      .hero-logo {
        top: 18px; bottom: auto; left: 50%; transform: translateX(-50%);
        height: 96px;
      }
      .hero-logo:hover { transform: translateX(-50%) translateY(-1px); }
    }
    @media print { .hero-logo { box-shadow: none; } }
...
<div class="hero">
  <a class="hero-logo" href="mbi_annonces_index.php" aria-label="Ma Box Immo — Accueil">
    <img src="images/mbi_annonces_logo.png" alt="Ma Box Immo">
  </a>
