...
$stmtImm = $pdo->prepare("SELECT id, nom_immeuble AS nom, reference_immeuble AS reference FROM immeubles WHERE id_societe = ? OR id_societe IS NULL ORDER BY nom");
$stmtImm->execute([$societeId]);
$immeubles = $stmtImm->fetchAll(PDO::FETCH_ASSOC);
$stmtUsr = $pdo->prepare("SELECT id, nom, prenom FROM users WHERE actif=1 AND (id_societe = ? OR id_societe IS NULL) ORDER BY nom,prenom");
$stmtUsr->execute([$societeId]);
$all_users = $stmtUsr->fetchAll(PDO::FETCH_ASSOC);
