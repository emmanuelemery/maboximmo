...
if ($isSuperAdmin) {
    $proprietaires = $pdo->query("
        SELECT p.id AS id_proprietaire, COALESCE(p.societe,CONCAT(p.prenom,' ',p.nom)) AS label
        FROM proprietaires p ORDER BY label
    ")->fetchAll(PDO::FETCH_ASSOC);
} else {
    $stmtProp = $pdo->prepare("
        SELECT up.id_proprietaire, COALESCE(up.label, p.societe, CONCAT(p.prenom,' ',p.nom)) AS label
        FROM user_proprietaires up JOIN proprietaires p ON p.id=up.id_proprietaire
        WHERE up.id_user=? ORDER BY up.ordre
    ");
...
/* ── Filtre propriétaire actif ─────────────────────────────
   Priorité : 1) GET ?prop=  2) Session bailleur_props  3) Auto si 1 seul
*/
$fSearch = trim($_GET['q'] ?? '');
$fStatut = in_array($_GET['statut'] ?? '', ['actif','vendu','archive','tous'], true)
    ? $_GET['statut'] : 'actif';
...
if (isset($_GET['prop'])) {
    ...
    if ($fProp > 0) $_SESSION['bailleur_props'] = [$fProp];
    else            $_SESSION['bailleur_props'] = [];
} elseif (!empty($_SESSION['bailleur_props'])) {
    // Reprendre la sélection du dashboard (premier item)
    $fProp = (int)($_SESSION['bailleur_props'][0] ?? 0);
}

// Auto-select si un seul proprio
if ($fProp <= 0 && count($propIds) === 1) $fProp = (int)$propIds[0];

// Valider que le fProp est accessible
if ($fProp > 0 && !in_array($fProp, $propIds)) $fProp = 0;

$activePropIds = ($fProp > 0) ? [$fProp] : $propIds;
...
/* ── Layout ── */
$layout_title = 'Immeubles Bailleur'; $layout_module = 'Ma Box Bailleur'; $layout_sidebar = 'sidebar_bailleur_module';
