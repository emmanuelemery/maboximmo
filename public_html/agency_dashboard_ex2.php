...
// ── Stats FluxBox + tâches/GED (graceful fallback si tables absentes) ──
$fbxStats = ['pending'=>0, 'urgent'=>0, 'later'=>0, 'today_validated'=>0, 'doublons_blocked'=>0];
$gedToReview = 0;
$tachesAFaire = 0;
$fluxboxReady = false;
try {
    require_once __DIR__ . '/inc/fluxbox_functions.php';
    $fbxStats = fluxbox_carte_stats($pdo);
    $fluxboxReady = true;
} catch (Throwable) { /* table fluxbox_cartes pas encore migrée */ }

try {
    $st = $pdo->prepare("SELECT COUNT(*) FROM ged_documents WHERE tenant_id = ? AND status IN ('draft','active') AND COALESCE(JSON_EXTRACT(metadata, '$.to_review'), 0) = 1");
    $st->execute([(int)($_SESSION['id_societe'] ?? 0)]);
    $gedToReview = (int)$st->fetchColumn();
} catch (Throwable) {}

try {
    // Table = `taches` (pas agency_taches). Filtrage par société via jointure immeubles.
    // Manager (role >= 2) ne voit que ses tâches (créées ou assignées) — cohérent avec agency_taches.php.
    ...
    if ($roleId >= 2 && $userId > 0) {
        $st = $pdo->prepare("
            ...
            WHERE (i.id_societe = ? OR i.id IS NULL)
              AND t.statut NOT IN ('terminee','terminée','archivee','archivée')
              AND (t.id_createur = ? OR t.id IN (SELECT id_tache FROM taches_utilisateurs WHERE id_utilisateur = ?))
        ");
        $st->execute([$societeId, $userId, $userId]);
    } else {
        $st = $pdo->prepare("
            SELECT COUNT(*) FROM taches t
            LEFT JOIN immeubles i ON i.id = t.id_immeuble
    ...
    $tachesAFaire = (int)$st->fetchColumn();
} catch (Throwable) {}
...
/* ── FluxBox HERO (bandeau prioritaire tout en haut, full width) ── */
.ad-fluxbox-hero {
    display: block;
    width: 100%;
    background: linear-gradient(135deg, #243B5C 0%, #1e3050 50%, #324A6E 100%);
    border-radius: 22px;
    padding: 22px 28px;
    margin-bottom: 24px;
    color: #fff;
    text-decoration: none;
    box-shadow: 10px 10px 26px rgba(36,59,92,0.22), -4px -4px 14px #fff;
    transition: transform .2s cubic-bezier(0.22,1,0.36,1), box-shadow .2s;
    position: relative;
    overflow: hidden;
}
.ad-fluxbox-hero::before {
    content: "";
    position: absolute;
    top: -40%; right: -10%;
    width: 320px; height: 320px;
    background: radial-gradient(circle, rgba(212,160,71,0.18) 0%, transparent 70%);
    pointer-events: none;
}
.ad-fluxbox-hero:hover {
    transform: translateY(-3px);
    box-shadow: 14px 14px 34px rgba(36,59,92,0.32), -4px -4px 14px #fff;
}
.ad-fluxbox-hero-inner {
    display: flex;
    align-items: center;
    gap: 24px;
    flex-wrap: wrap;
...
.ad-fluxbox-hero-icon {
    width: 56px; height: 56px;
    background: linear-gradient(135deg, #D4A047, #b88835);
    border-radius: 16px;
    display: flex; align-items: center; justify-content: center;
    font-size: 28px;
    box-shadow: inset 2px 2px 5px rgba(255,255,255,0.2), 3px 3px 8px rgba(0,0,0,0.18);
    flex-shrink: 0;
}
.ad-fluxbox-hero-text {
    min-width: 0;
    flex: 1;
}
.ad-fluxbox-hero-title {
...
.ad-fluxbox-hero-subtitle {
    font-size: 13px;
    color: rgba(255,255,255,0.78);
    margin-top: 2px;
}
.ad-fluxbox-hero-kpis {
    display: flex;
    gap: 14px;
    flex-wrap: wrap;
    align-items: center;
...
.ad-fluxbox-kpi.urgent .ad-fluxbox-kpi-value { color: #FCA5A5; }
.ad-fluxbox-kpi.urgent { border-color: rgba(220,38,38,0.4); background: rgba(220,38,38,0.18); }
.ad-fluxbox-kpi.gold .ad-fluxbox-kpi-value { color: #FCD34D; }
.ad-fluxbox-hero-cta {
    background: linear-gradient(135deg, #D4A047, #b88835);
    color: #1a1816;
    font-weight: 700;
...
.ad-fluxbox-hero-cta-arrow { font-size: 16px; transition: transform .2s; }
.ad-fluxbox-hero:hover .ad-fluxbox-hero-cta-arrow { transform: translateX(4px); }
@media (max-width: 760px) {
    .ad-fluxbox-hero-inner { flex-direction: column; align-items: stretch; gap: 16px; }
    .ad-fluxbox-hero-kpis { justify-content: space-between; }
    .ad-fluxbox-kpi { flex: 1; min-width: 60px; padding: 8px 8px; }
    .ad-fluxbox-hero-cta { margin-left: 0; justify-content: center; }
}
...
<div class="ad-wrap">
    ...
    <!-- ── FluxBox HERO : pile de cartes à valider en priorité ── -->
    <a href="./fluxbox.php" class="ad-fluxbox-hero">
        <div class="ad-fluxbox-hero-inner">
            <div class="ad-fluxbox-hero-left">
                <div class="ad-fluxbox-hero-icon">🃏</div>
                <div class="ad-fluxbox-hero-text">
                    <h2 class="ad-fluxbox-hero-title">FluxBox</h2>
                    <div class="ad-fluxbox-hero-subtitle">
                        <?php if (!$fluxboxReady): ?>
                            Pile de cartes à valider — pas encore activée
                        <?php elseif ((int)$fbxStats['pending'] === 0 && $gedToReview === 0 && $tachesAFaire === 0): ?>
                            Tout est traité. Profitez-en.
                        <?php else: ?>
                            <?php
                                $total = (int)$fbxStats['pending'] + $gedToReview + $tachesAFaire;
                                echo "$total chose" . ($total > 1 ? 's' : '') . " en attente — environ " . max(1, (int)round($total * 0.5)) . " min";
                            ?>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            <div class="ad-fluxbox-hero-kpis">
                <?php if ((int)$fbxStats['urgent'] > 0): ?>
                <div class="ad-fluxbox-kpi urgent">
                    <div class="ad-fluxbox-kpi-value">🔴 <?= (int)$fbxStats['urgent'] ?></div>
                    <div class="ad-fluxbox-kpi-label">Urgent</div>
                </div>
                <?php endif; ?>
                <div class="ad-fluxbox-kpi">
                    <div class="ad-fluxbox-kpi-value"><?= (int)$fbxStats['pending'] ?></div>
                    <div class="ad-fluxbox-kpi-label">À valider</div>
                </div>
                <div class="ad-fluxbox-kpi">
                    <div class="ad-fluxbox-kpi-value"><?= $tachesAFaire ?></div>
                    <div class="ad-fluxbox-kpi-label">Tâches</div>
                </div>
                <div class="ad-fluxbox-kpi">
                    <div class="ad-fluxbox-kpi-value"><?= $gedToReview ?></div>
                    <div class="ad-fluxbox-kpi-label">GED</div>
                </div>
                <div class="ad-fluxbox-kpi gold">
                    <div class="ad-fluxbox-kpi-value">✅ <?= (int)$fbxStats['today_validated'] ?></div>
                    <div class="ad-fluxbox-kpi-label">Validés</div>
                </div>
                <?php if ((int)$fbxStats['doublons_blocked'] > 0): ?>
                <div class="ad-fluxbox-kpi">
                    <div class="ad-fluxbox-kpi-value">🛡️ <?= (int)$fbxStats['doublons_blocked'] ?></div>
                    <div class="ad-fluxbox-kpi-label">Doublons</div>
                </div>
                <?php endif; ?>
            </div>
            <div class="ad-fluxbox-hero-cta">
                Ouvrir FluxBox
                <span class="ad-fluxbox-hero-cta-arrow">→</span>
            </div>
        </div>
    </a>
