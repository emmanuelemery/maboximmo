<?php
/**
 * creancier_dossier360.php — 360 d'UN dossier créancier (détail complet, lecture seule).
 *
 ...
 *
 * Usage : /creancier_dossier360.php?id_dossier=1
 */
...
// ── Agenda du dossier (dates extraites + saisies + items) ────────────
...
<div class="cre-wrap" style="--risk: <?= $riskColor ?>;">
  ...
  <div class="cre-grid">
    ...
    <div class="cre-card">
      ...
      <?php foreach ($acteurs as $a): $isCrea = str_starts_with((string)$a['role_dossier'], 'creancier'); ?>
        <div class="cre-row"><span><?php if ($isCrea): ?><a href="creancier_creancier360.php?id=<?= (int)$a['entity_id'] ?>" style="color:#243B5C;font-weight:600;text-decoration:none"><?= h($a['tiers_lib']) ?> 🔗</a><?php else: ?><?= h($a['tiers_lib']) ?><?php endif; ?> <?php if ($a['role_dossier']): ?><span class="cre-pill"><?= h($a['role_dossier']) ?></span><?php endif; ?></span>
    ...
    <div class="cre-card">
      ...
      <?php foreach ($sociétés as $s): ?>
        <div class="cre-row"><span><span class="cre-pill">SOCIÉTÉ</span> <a href="creancier360.php?type=SOCIETE&id=<?= (int)$s['entity_id'] ?>" style="color:#243B5C;font-weight:600;text-decoration:none"><?= h($s['soc_lib'] ?: ('#' . $s['entity_id'])) ?> 🔗</a></span>
    ...
    <div class="cre-card" style="border-left:6px solid #4878a6">
      <h3>📅 Agenda du dossier</h3>
      <?php if (!$agenda): ?><div class="cre-empty">Aucune date.</div><?php endif; ?>
      <?php foreach ($agenda as $ev): ?>
        <div class="cre-row"><span><span class="cre-pill"><?= h($ev['type']) ?></span> <?= h($ev['libelle']) ?></span>
          <span style="font-weight:700;color:<?= $ev['retard'] ? '#dc2626' : '#4878a6' ?>"><?= $dfr($ev['date']) ?></span></div>
      <?php endforeach; ?>
    </div>
