...
<!-- Zone upload — 2 méthodes en parallèle (zéro régression) -->
<div class="bdl-upload">
    <span class="lbl">📤 Charger un ou plusieurs documents sur ce bien</span>

    <!-- Méthode 1 : Modal FluxBox (recommandée, pipeline auto IA + V3.1 + auto-commit) -->
    <button type="button" id="bdlOpenFluxModal"
            style="background:#7c3aed;color:#fff;border:none;padding:12px 22px;border-radius:6px;font-weight:700;font-size:14px;cursor:pointer;box-shadow:2px 2px 6px rgba(124,58,237,0.4);margin-bottom:10px;display:inline-flex;align-items:center;gap:8px;">
        🚀 Charger via Modal FluxBox (IA auto + classement V3.1)
    </button>

    <details style="margin-top:8px;">
        <summary style="cursor:pointer;font-size:11px;color:#92400e;opacity:0.8;">Méthode classique (upload simple, review manuelle) ↓</summary>
        <form method="POST" action="api/transaction_quick_upload.php" enctype="multipart/form-data" style="margin-top:10px;">
            ...
            <button type="submit">📤 Upload + Review (méthode legacy)</button>
        </form>
    </details>
...
<script>
// Configure le prefill FluxBox depuis le contexte bien
window.FBX_PREFILL = {
    bien_id:         <?= (int)$bienId ?>,
    soc_id:          <?= (int)($bien['id_societe'] ?? 1) ?>,
    age_id:          <?= (int)($bien['id_agence']  ?? 3) ?>,
    proprio_tiers_id:<?= (int)$proprioTiersId ?>,
    immeuble_id:     <?= (int)($bien['id_immeuble'] ?? 0) ?>,
    n1:              '03_GESTION_LOCATIVE',
    n2:              'BIENS',
    n3:              'BIEN',
    entite_nom:      <?= json_encode(($bien['designation'] ?? '') ?: ($bien['adresse_1'] ?? '') ?: ('Bien #' . $bienId), JSON_UNESCAPED_UNICODE) ?>,
    entite_id_bdd:   <?= (int)$bienId ?>,
    entite_adresse:  <?= json_encode(($bien['adresse_1'] ?? '') . ($bien['code_postal'] ? ' · ' . $bien['code_postal'] . ' ' . ($bien['ville'] ?? '') : ''), JSON_UNESCAPED_UNICODE) ?>,
    proprio_nom:     <?= json_encode($proprioLabel, JSON_UNESCAPED_UNICODE) ?>,
    origin:          'bien_documents_list',
};

document.getElementById('bdlOpenFluxModal')?.addEventListener('click', function() {
...
<?php /* Fix B1 (2026-05-26) : fluxbox_upload_modal.php est inclus via agency_layout_bottom — pas de doublon ici */ ?>
