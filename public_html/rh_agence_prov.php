<?php
/**
 * rh_agence_prov.php — Page « Mon agence » SIMPLIFIÉE (provisoire).
 *
 * Duplicat de `rh_agence_user.php` pour aligner le nommage des pages de
 * premier accès collaborateur (suffixe _prov). Identique fonctionnellement :
 *   - Bloc lecture seule : informations d'inscription (nom, SIRET, RCS…)
 *   - Bloc éditable      : coordonnées de contact + horaires + commentaire
 *   - Bloc validation    : bouton « Je valide les coordonnées »
 *                          → users.user_agence_confirmed_at = NOW()
 *                          → retour dashboard avec flash.
 *
 * Utilise la sidebar_user (aucun lien admin exposé).
 */
...
$layout_head_kpis = '
<div class="ph-kpi" style="min-width:200px;flex-direction:row;gap:10px;align-items:center;padding:6px 16px;white-space:nowrap">
    <span style="font-size:16px">🏢</span>
    <div><div class="ph-kpi-lbl">Agence</div><div class="ph-kpi-val" style="font-size:12px">' . htmlspecialchars($agence['nom_agence'] ?? '—', ENT_QUOTES, 'UTF-8') . '</div></div>
</div>
<div class="ph-kpi" style="min-width:200px;flex-direction:row;gap:10px;align-items:center;padding:6px 16px;white-space:nowrap">
    <span style="font-size:16px">📍</span>
    <div><div class="ph-kpi-lbl">Ville</div><div class="ph-kpi-val" style="font-size:12px">' . htmlspecialchars(($agence['code_postal'] ?? '') . ' ' . ($agence['ville'] ?? ''), ENT_QUOTES, 'UTF-8') . '</div></div>
</div>
<div class="ph-kpi" style="min-width:160px;flex-direction:row;gap:10px;align-items:center;padding:6px 16px;white-space:nowrap">
    <span style="font-size:16px">📞</span>
    <div><div class="ph-kpi-lbl">Téléphone</div><div class="ph-kpi-val" style="font-size:12px">' . htmlspecialchars($agence['telephone'] ?? '—', ENT_QUOTES, 'UTF-8') . '</div></div>
</div>
';
...
<!-- Bloc lecture seule supprimé — infos déplacées dans le page-head -->
