...
function analyseBailIA(string $text): array
{
    ...
    // Force gpt-4o-mini pour l'extraction structurée : rapide, précis, JSON fiable.
    // Les modèles reasoning (gpt-5, o1, o3) gaspillent les tokens en "thinking"
    // → JSON tronqué + très lent. Cf. memo dpe_ia_analyse.php.
    $model = defined('OPENAI_BAIL_MODEL') ? OPENAI_BAIL_MODEL : 'gpt-4o-mini';
...
PROMPT;
    ...
    // Détection modèle "reasoning" (gpt-5, o1, o3, gpt-4.1) qui exige
    // max_completion_tokens à la place de max_tokens et n'accepte plus temperature.
    ...
    if ($useNewParam) {
        $payloadArr['max_completion_tokens'] = 3500;
    } else {
        $payloadArr['max_tokens']  = 3500;
        $payloadArr['temperature'] = 0.0;
    }
    ...
    if ($httpCode !== 200) {
        ...
        error_log('[analyseBailIA] HTTP ' . $httpCode . ' OpenAI : ' . $apiMsg);
        return ['ok' => false, 'fields' => [], 'doc_type' => 'bail',
