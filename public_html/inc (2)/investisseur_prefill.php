...
if (!function_exists('inv_listable_biens')) {
    ...
    function inv_listable_biens(PDO $pdo, int $limit = 500): array {
        ...
        $sql = "SELECT b.id, b.reference_bien, b.designation, b.adresse_1, b.ville, b.code_postal,
                       b.surface_habitable, b.statut_occupation, b.prix_vente_estime, b.loyer_hc,
                       tb.libelle AS type_libelle,
                       b.id_proprietaire,
                       COALESCE(NULLIF(p.societe, ''), NULLIF(CONCAT_WS(' ', p.civilite, p.prenom, p.nom), ''), 'Sans propriétaire') AS proprio_label
                FROM biens b
                LEFT JOIN types_bien tb ON tb.id = b.id_type_bien
                LEFT JOIN proprietaires p ON p.id = b.id_proprietaire
                WHERE $where
                ORDER BY proprio_label ASC, b.ville ASC, b.adresse_1 ASC, b.id DESC
                LIMIT " . (int)$limit;
        $st = $pdo->prepare($sql);
        $st->execute($params);
