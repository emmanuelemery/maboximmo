...
function analyseBienIntakeIA(string $text, ?string $forceType = null): array
{
    ...
    // EXCEPTION 'divers' : l'utilisateur dit "je ne sais pas, débrouille-toi".
    // On lance la détection regex ; si elle reconnaît clairement un bail / mandat /
    // diag / titre / fiche, on route vers ce module au lieu de tag aveuglément divers.
    // Sinon (vraiment ambigu) → reste sur divers et utilise le pipeline générique.
    $validForced = ['bail', 'mandat', 'titre', 'diag', 'fiche', 'divers'];
    if ($forceType !== null) {
        $forceType = strtolower(trim($forceType));
        if ($forceType === 'divers') {
            $detected = detectBienIntakeDocType($text);
            $docType  = ($detected !== 'inconnu') ? $detected : 'divers';
        } elseif (in_array($forceType, $validForced, true)) {
            $docType = $forceType;
