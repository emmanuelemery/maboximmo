...
$st = $pdo->prepare("SELECT bb.id, bb.id_societe, bb.bail_nature, b.id_proprietaire
...
// Dispatch selon la nature : habitation → moteur FNAIM 89-462 ; sinon → commercial.
$estHabitation = in_array((string)($row['bail_nature'] ?? ''), ['habitation', 'meuble_touristique'], true);
try {
    if ($estHabitation && is_file(dirname(__DIR__) . '/inc/bail_habitation_pdf.php')) {
        require_once dirname(__DIR__) . '/inc/bail_habitation_pdf.php';
        $path = bail_habitation_build_pdf($pdo, $bailId, $forceProjet);
    } else {
