...
try {
    ...
    $updateStmt = $pdo->prepare("
        UPDATE conges
        SET statut = 'validé', date_validation = NOW(), id_validateur = ?
        WHERE id = ?
    ");
    $updateStmt->execute([$userId, $congeId]);

    echo json_encode(['success' => true, 'message' => 'Congé validé avec succès']);
} catch (Exception $e) {
    error_log("Error validating conge: " . $e->getMessage());
    http_response_code(500);
