...
foreach ($depots as $d) {
    ...
    $r = rhb_pages_par_salarie($d['chemin'], $sal);
    $ligne['ok']             = (bool)$r['ok'];
    $ligne['error']          = $r['error'];
...
$nbSal = 0; $nbPagesOrphelines = 0; $nbPresume = 0;
foreach ($analyse as $a) {
    $nbSal += count($a['salaries']);
    $nbPagesOrphelines += count($a['non_attribuees']);
    foreach ($a['salaries'] as $s) if (!empty($s['presume'])) $nbPresume++;
}
...
if ($mode === 'send') {
    ...
    foreach ($analyse as $a) {
        ...
        foreach ($a['salaries'] as $s) {
            ...
            if ($ok) $envoyes++; else $echecs[] = $s['nom'] . ' : envoi refusé par le serveur de messagerie';
            @unlink($pdfEnvoi);
        }
    }
...
$cree = 0; $ignore = 0; $erreurs = [];

$dejaLa = $pdo->prepare("SELECT id FROM rh_documents
                          WHERE id_user = ? AND type_document = 'bulletin_paie' AND label = ? AND actif = 1 LIMIT 1");
$insert = $pdo->prepare(
    "INSERT INTO rh_documents (id_user, id_societe, id_agence, categorie, sous_categorie, type_document,
                               label, filename, original_name, file_path, taille, mime_type,
                               uploaded_by, actif, metadata_json)
     VALUES (?,?,?, 'rh', 'bulletin_paie', 'bulletin_paie', ?,?,?,?,?, 'application/pdf', ?, 1, ?)");
...
foreach ($analyse as $a) {
    ...
    foreach ($a['salaries'] as $s) {
        $uid   = (int)$s['id_user'];
        $label = sprintf('Bulletin de paie %02d/%04d', $mois, $annee);

        // Rejouer la découpe ne doit pas empiler les doublons dans le dossier du salarié.
        $dejaLa->execute([$uid, $label]);
        if ($dejaLa->fetchColumn()) { $ignore++; continue; }

        $dossier = $racine . '/uploads/rh_docs/' . $uid;
        if (!is_dir($dossier)) @mkdir($dossier, 0777, true);
        $nomFichier = sprintf('bulletin_%04d-%02d_%s.pdf', $annee, $mois, bin2hex(random_bytes(5)));
        $abs = $dossier . '/' . $nomFichier;

        $ex = rhb_extraire_pages($a['_chemin'], $s['pages'], $abs);
        if (!$ex['ok']) { $erreurs[] = $s['nom'] . ' : ' . ($ex['error'] ?? 'extraction impossible'); continue; }

        try {
            $insert->execute([
                $uid, $a['id_societe'] ?: null, $a['id_agence'] ?: null,
                $label, $nomFichier,
                sprintf('%s_%04d-%02d.pdf', rhb_slug($s['nom']), $annee, $mois),
                '/uploads/rh_docs/' . $uid . '/' . $nomFichier,
                filesize($abs), $userId,
                json_encode(['mois'=>$mois, 'annee'=>$annee, 'depot_id'=>$a['depot_id'],
                             'pages'=>$s['pages'], 'suite_presumee'=>(bool)$s['presume']], JSON_UNESCAPED_UNICODE),
            ]);
            $cree++;
        } catch (Throwable $e) {
            @unlink($abs);          // pas de fichier orphelin si la ligne n'a pas pu s'écrire
            $erreurs[] = $s['nom'] . ' : ' . $e->getMessage();
        }
    }
}
...
echo json_encode([
    'ok'      => true,
    'crees'   => $cree,
    'ignores' => $ignore,                 // déjà présents pour ce mois
    'a_verifier' => $nbPagesOrphelines,   // pages jamais écrites, à traiter à la main
    'erreurs' => $erreurs,
    'message' => $cree . ' bulletin(s) rangé(s)'
                 . ($ignore ? ', ' . $ignore . ' déjà présent(s)' : '')
