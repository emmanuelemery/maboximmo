...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['close_month'])) {
    ...
    /* CLÔTURE PAR SOCIÉTÉ (2026-08-11).
       Auparavant : UPDATE salaires … WHERE mois_reference LIKE ? — sans aucun
       filtre. Clôturer la Régie Emery verrouillait donc la paie de Loca Immo et
       d'Emery Immo, et le bouton affichait « Mois cloture » pour tout le monde.
       La portée vient de la société SÉLECTIONNÉE à l'écran (elle est dans la
       query string, que le POST conserve). 0 = toutes celles du périmètre. */
    $socCloture = (int)($_GET['societe'] ?? 0);      // 'toutes' → (int) = 0
    if (!in_array($roleId, [1, 7], true)) {
        // Un admin de société ne clôture jamais au-delà de la sienne.
        $socCloture = (int)($_SESSION['id_societe'] ?? 0);
        if ($socCloture <= 0) { http_response_code(403); exit('Société de rattachement inconnue'); }
    }

    require_once __DIR__ . '/inc/rh_cloture.php';
    $resCloture = rhc_cloturer($pdo, $mois, $annee, $socCloture);
    if (!$resCloture['ok']) {
        http_response_code(400);
        exit('Clôture impossible : ' . ($resCloture['error'] ?? 'erreur inconnue'));
    }

    http_response_code(200);
    exit('Mois clôturé — ' . $resCloture['salaires'] . ' salaire(s), '
         . count($resCloture['societes']) . ' société(s)');
}
...
/* Le mois est-il clôturé POUR LA SOCIÉTÉ AFFICHÉE ?
   Le test portait avant sur toute la table `salaires`, sans filtre : dès qu'une
   société clôturait, « Mois cloture » s'affichait pour les autres. La portée est
   désormais explicite (inc/rh_cloture.php).
   « toutes sociétés » sélectionné → clos seulement si TOUTES le sont, sans quoi
   le bouton doit rester actionnable pour celles qui restent. */
require_once __DIR__ . '/inc/rh_cloture.php';
$mois_ref_check = sprintf('%04d-%02d-01', (int)$annee_sel, (int)$mois_sel);
$societeCloture = ($societe_sel !== 'toutes') ? (int)$societe_sel : 0;
$monthClosed    = rhc_est_clos($pdo, (int)$mois_sel, (int)$annee_sel, $societeCloture);
// Ce qu'il reste à clôturer quand on regarde tout le périmètre (info du bouton).
$societesOuvertes = $societeCloture === 0
    ? (rhc_societes_ouvertes($pdo, (int)$mois_sel, (int)$annee_sel) ?? [])
    : [];
...
/* ── Card 4 : BULLETINS CORRIGÉS ───────────────────────────────────────────────
   Un ou plusieurs bulletins déjà réduits à un salarié (ou un PDF en contenant
   plusieurs). Ils sont classés au coffre en nouvelle version, et RIEN d'autre :
   aucune ligne `rh_salaires_comparaisons`, donc le dépôt de référence de l'agence,
   le total net et la comparaison avec l'attendu restent intacts. C'est toute la
   différence avec la card 3, où un bulletin unique écraserait la photo comptable
   du mois. Le salarié est identifié par le PDF lui-même : aucune agence à régler. */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['upload_bulletins_corriges'])) {
    verify_csrf();
    if (!$rhAdmin) { http_response_code(403); exit('Accès refusé.'); }
    ...
    /* Périmètre d'identification : la société affichée d'abord. Si le PDF n'y
       correspond à personne, on élargit — un bulletin déposé depuis le mauvais
       menu ne doit pas être refusé pour cette seule raison. L'élargissement reste
       réservé au super admin : un admin de société ne classe que chez lui. */
    $salSociete = $societeId > 0 ? rhb_salaries($pdo, $societeId, 0) : [];
    $peutElargir = in_array($roleId, [1, 7], true);
    $salLarge = null;
    ...
    foreach ((array)$files['name'] as $i => $nomOrig) {
        ...
        if ((empty($an['ok']) || empty($an['par_salarie'])) && $peutElargir) {
            if ($salLarge === null) $salLarge = rhb_salaries($pdo, 0, 0);
            $liste = $salLarge;
        ...
        foreach ($an['par_salarie'] as $ligneSal) {
            ...
            $sal = null;
            foreach ($liste as $s) { if ((int)$s['id'] === $uid) { $sal = $s; break; } }
            if (!$sal) continue;

            // Société/agence du SALARIÉ, pas de l'écran : le bulletin suit son titulaire.
            $qCtx = $pdo->prepare("SELECT u.id_societe, u.id_agence, COALESCE(so.nom,'') AS soc,
                                          COALESCE(a.nom_agence,'') AS age
                                     FROM users u
                                LEFT JOIN societes so ON so.id = u.id_societe
                                LEFT JOIN agences  a  ON a.id  = u.id_agence
                                    WHERE u.id = ? LIMIT 1");
            $qCtx->execute([$uid]);
            $ctxSal = $qCtx->fetch(PDO::FETCH_ASSOC) ?: [];

            $pagesSal = array_map('intval', (array)$ligneSal['pages']);
            $viaSal   = (string)($an['detail'][$pagesSal[0]]['via'] ?? '');
            $depotUnitaire = [
...
<style>
    ...
    /* 5 étapes sur une ligne : la 1re ne porte qu'un état d'envoi, sans saisie —
       elle cède sa largeur aux cards qui reçoivent un fichier. */
    @media (min-width: 1360px) {
        .workflow-actions { grid-template-columns: 0.7fr 1fr 1fr 1fr 1fr; }
    }
    .workflow-step {
        background: var(--bg-primary, #ffffff);
        border-radius: 14px;
...
<div class="action-strip">
    ...
    <div class="action-strip-btns">
        ...
        <?php if ($rhAdmin): ?>
            ...
            <?php
            /* Bulletins de paie — deux gestes distincts, volontairement séparés :
                 · récupérer les PDF déposés par le comptable (une agence = un PDF) ;
                 · les découper en un bulletin par salarié.
               Le second écrit dans les dossiers RH : il passe par un aperçu validé
               à la main, jamais en un clic. */
            $bullScope = ($societe_sel !== 'toutes') ? (int)$societe_sel : 0;
            $bullQs = 'soc=' . $bullScope . '&mois=' . (int)$mois_sel . '&annee=' . (int)$annee_sel
                    . ($agence_sel !== 'toutes' ? '&agence=' . (int)$agence_sel : '');
            ?>
            <?php /* Un SEUL point d'entrée : le choix « par agence » ou « par salarié »
                     se fait dans la fenêtre, avec la société, le mois et l'année. */ ?>
            <button type="button" class="v2-btn" style="background:#0f766e;color:#fff;border-color:#0f766e;"
                    onclick="ouvrirTelechargementBulletins()"
                    title="Récupérer les bulletins : par agence, ou découpés par salarié — choix société / mois / année dans la fenêtre">
                📥 Bulletins
            </button>

            <?php
            /* La clôture est PAR SOCIÉTÉ depuis le 2026-08-11. L'infobulle dit la
               portée exacte : sans cela « Mois cloture » laisse croire à un verrou
               global, ce qu'il n'est plus. */
            $titreCloture = $monthClosed
                ? ($bullScope ? 'Mois clôturé pour cette société' : 'Mois clôturé pour toutes les sociétés du périmètre')
                : ($bullScope ? 'Clôturer le mois pour cette société uniquement'
                              : 'Clôturer le mois pour ' . (count($societesOuvertes) ?: 'toutes les') . ' société(s) restante(s)');
            ?>
            <button onclick="closeMonth(<?=$mois_sel?>, <?=$annee_sel?>)"
                    class="v2-btn <?=$monthClosed ? 'locked' : 'danger'?>"
                    <?=$monthClosed ? 'disabled' : ''?>
                    title="<?=h($titreCloture)?>"
                    <?php if ($monthClosed): ?>style="background:#e2e8f0;color:#475569;border-color:#cbd5e1;font-weight:800;letter-spacing:.04em;cursor:not-allowed;"<?php endif; ?>>
...
<?php if ($rhAdmin): ?>
<?php
/* Périmètre proposé au téléchargement. Un admin de société ne se voit offrir que
   la sienne : la restriction est de toute façon refaite côté API, mais proposer un
   choix qui sera refusé est une mauvaise manière. */
$dlbSuperAdmin = in_array($roleId, [1, 7], true);
/* La société de rattachement vient de `users` — c'est la source dont la page se
   sert déjà ($user_societe_default). La session peut en diverger ; s'aligner sur
   elle donnerait un modal qui propose une société et une page calée sur une autre,
   sans aucune pastille active. */
$dlbSocUser = (int)($user_societe_default !== 'toutes'
    ? $user_societe_default
    : ($_SESSION['id_societe'] ?? 0));
$dlbSocietes = $dlbSuperAdmin
    ? $societes
    : array_values(array_filter($societes, fn($s) => (int)$s['id'] === $dlbSocUser));
// La présélection doit toujours désigner une pastille réellement proposée.
$dlbChoixSoc = (int)$bullScope;
if (!$dlbSuperAdmin && $dlbChoixSoc !== $dlbSocUser) $dlbChoixSoc = $dlbSocUser;
$dlbAnnees = range((int)$annee_sel - 2, (int)$annee_sel + 1);
$dlbMoisCourt = ['','janv','févr','mars','avr','mai','juin','juil','août','sept','oct','nov','déc'];
?>
<!-- ── Modal « Télécharger les bulletins » : société / mois / année en un coup d'œil ──
     Le bouton ne télécharge plus le périmètre affiché en silence : on choisit ici,
     en trois clics, sans quitter la page ni toucher aux filtres de l'écran. -->
<div id="dlb-modal" style="display:none;position:fixed;inset:0;background:rgba(15,23,42,.55);z-index:9999;align-items:center;justify-content:center;padding:14px;" onclick="if(event.target===this)fermerTelechargementBulletins()">
    <?php /* Large : en mode « par salarié » la fenêtre se scinde en deux, la liste
             à gauche et le PDF réellement produit à droite. */ ?>
    <div id="dlb-boite" style="background:#fff;border-radius:14px;max-width:660px;width:100%;max-height:90vh;display:flex;flex-direction:column;box-shadow:0 20px 60px rgba(0,0,0,.4);overflow:hidden;transition:max-width .15s;">
        <div style="padding:14px 20px;border-bottom:1px solid #e5e7eb;display:flex;justify-content:space-between;align-items:center;">
            <h3 style="margin:0;font-size:15px;color:#243B5C;">📥 Télécharger les bulletins</h3>
            <button type="button" onclick="fermerTelechargementBulletins()" style="background:transparent;border:none;font-size:20px;cursor:pointer;color:#64748b;">×</button>
        </div>
        <div style="padding:12px 18px;overflow:auto;flex:1;">
            <?php /* Deux lignes de filtres, pas cinq : chaque ligne gagnée ici est une
                     ligne rendue à l'aperçu du bulletin, qui est ce qu'on vient lire. */ ?>
            <div class="dlb-filtres">
                <span class="dlb-lbl">Société</span>
                <?php if ($dlbSuperAdmin): ?>
                <button type="button" class="filter-pill dlb-large" data-champ="soc" data-val="0">Toutes</button>
                <?php endif; ?>
                <?php foreach ($dlbSocietes as $s): ?>
                <button type="button" class="filter-pill dlb-large" data-champ="soc" data-val="<?=(int)$s['id']?>"><?=h($s['nom'])?></button>
                <?php endforeach; ?>
                <span class="dlb-sep"></span>
                <?php /* Le libellé est indispensable : sans lui, ces deux pills se
                         lisent comme deux sociétés de plus, et l'accès au détail par
                         salarié devient introuvable. */ ?>
                <span class="dlb-lbl">Contenu</span>
                <button type="button" class="filter-pill dlb-large dlb-mode" data-champ="mode" data-val="0">📦 Un PDF par agence</button>
                <button type="button" class="filter-pill dlb-large dlb-mode" data-champ="mode" data-val="1">👤 Un PDF par salarié</button>
            </div>

            <div class="dlb-filtres">
                <span class="dlb-lbl">Mois</span>
                <?php for ($m = 1; $m <= 12; $m++): ?>
                <button type="button" class="filter-pill dlb-mois" data-champ="mois" data-val="<?=$m?>"><?=$dlbMoisCourt[$m]?></button>
                <?php endfor; ?>
                <span class="dlb-sep"></span>
                <span class="dlb-lbl">Année</span>
                <?php foreach ($dlbAnnees as $a): ?>
                <button type="button" class="filter-pill dlb-annee" data-champ="annee" data-val="<?=$a?>"><?=$a?></button>
                <?php endforeach; ?>
            </div>

            <div id="dlb-resume" style="margin-top:16px;padding:10px 12px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;font-size:12.5px;color:#334155;"></div>

            <?php /* Aperçu du découpage. À droite, le PDF RÉELLEMENT produit pour
                     le salarié sélectionné — pas une reconstitution : le fichier
                     qui partira. */ ?>
            <div id="dlb-zone" style="display:none;margin-top:12px;gap:14px;align-items:flex-start;">
                <div id="dlb-apercu" style="flex:1;min-width:0;font-size:13px;color:#334155;"></div>
                <?php /* Le PDF est ce qu'on vient vérifier : il prend la moitié de la
                         largeur et toute la hauteur disponible, pas une vignette. */ ?>
                <div id="dlb-visu" style="display:none;width:52%;min-width:420px;border:1px solid #e2e8f0;border-radius:10px;overflow:hidden;background:#f8fafc;position:sticky;top:0;">
                    <div id="dlb-visu-titre" style="padding:7px 10px;border-bottom:1px solid #e2e8f0;font-size:12px;font-weight:800;color:#243B5C;display:flex;justify-content:space-between;align-items:center;gap:8px;">
                        <span>Aperçu</span>
                        <button type="button" onclick="fermerVisu()" style="background:transparent;border:none;cursor:pointer;color:#64748b;font-size:16px;line-height:1;">×</button>
                    </div>
                    <div id="dlb-visu-ctl" style="padding:6px 10px;font-size:11.5px;"></div>
                    <iframe id="dlb-visu-frame" style="width:100%;height:76vh;border:none;background:#fff;"></iframe>
                </div>
            </div>

            <?php /* Composition du message : on relit le texte AVANT qu'il ne parte,
                     et on complète les adresses manquantes au passage. */ ?>
            <div id="dlb-mail" style="display:none;margin-top:4px;">
                <div class="tm-field" style="margin-bottom:10px;">
                    <label style="display:block;font-size:11px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.05em;margin-bottom:4px;">Objet</label>
                    <input type="text" id="dlb-sujet" style="width:100%;padding:8px 10px;border:1px solid #cbd5e1;border-radius:8px;font-size:13px;">
                </div>
                <div class="tm-field">
                    <label style="display:block;font-size:11px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.05em;margin-bottom:4px;">Message</label>
                    <textarea id="dlb-corps" rows="9" style="width:100%;padding:9px 11px;border:1px solid #cbd5e1;border-radius:8px;font-size:13px;font-family:inherit;line-height:1.45;"></textarea>
                    <div style="font-size:11px;color:#94a3b8;margin-top:4px;">
                        Remplacés pour chaque salarié : <code>{{prenom}}</code> <code>{{nom}}</code> <code>{{mois}}</code> <code>{{annee}}</code>.
                        Le texte part tel quel, sans mise en forme.
                    </div>
                </div>
                <div style="font-size:11px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.05em;margin:14px 0 6px;">
                    Destinataires <span id="dlb-dest-compte" style="font-weight:400;text-transform:none;letter-spacing:0;"></span>
                </div>
                <div id="dlb-dest" style="max-height:34vh;overflow:auto;border:1px solid #e2e8f0;border-radius:10px;"></div>
                <div id="dlb-mail-alerte" style="margin-top:8px;font-size:12px;"></div>
            </div>
        </div>
        <div style="padding:12px 20px;border-top:1px solid #e5e7eb;display:flex;gap:8px;justify-content:flex-end;align-items:center;">
            <span id="dlb-etat" style="margin-right:auto;font-size:12px;color:#64748b;"></span>
            <button type="button" class="v2-btn" onclick="fermerTelechargementBulletins()">Annuler</button>
            <button type="button" class="v2-btn" id="dlb-zip" style="display:none;"
                    onclick="telechargerDecoupeZip()">📦 ZIP des bulletins découpés</button>
            <button type="button" class="v2-btn" id="dlb-retour" style="display:none;" onclick="fermerComposeur()">← Retour</button>
            <button type="button" class="v2-btn" id="dlb-confirmer" style="display:none;background:#b45309;color:#fff;border-color:#b45309;"
                    onclick="confirmerEnvoi()">✉️ Confirmer l'envoi</button>
            <button type="button" class="v2-btn" id="dlb-envoyer-tout" style="display:none;"
                    onclick="ouvrirComposeur(0)"
                    title="Rédiger le message, vérifier les adresses, puis envoyer à chaque salarié SON bulletin.">✉️ Envoyer à chacun</button>
            <button type="button" class="v2-btn" id="dlb-classer" style="display:none;background:#0f766e;color:#fff;border-color:#0f766e;"
                    onclick="classerDecoupe()">📥 À classer dans les dossiers</button>
            <button type="button" class="v2-btn" id="dlb-valider"
                    style="background:#0f766e;color:#fff;border-color:#0f766e;"
                    onclick="validerTelechargementBulletins()">📥 Télécharger le ZIP</button>
        </div>
    </div>
</div>
<style>
/* Les noms de société ne tiennent pas dans la pill de 68px des mois. */
.filter-pill.dlb-large { width: auto; padding: 0 14px; }

/* Filtres du modal : une ligne = un groupe, label inline pour ne pas manger
   de hauteur — celle-ci revient à l'aperçu du bulletin. */
.dlb-filtres { display: flex; flex-wrap: wrap; align-items: center; gap: 6px; margin-bottom: 8px; }
.dlb-lbl     { font-size: 10px; font-weight: 800; color: #94a3b8; text-transform: uppercase;
               letter-spacing: .06em; margin-right: 2px; }
.dlb-sep     { flex: 0 0 1px; align-self: stretch; background: #e2e8f0; margin: 2px 8px; }

/* Mois, année, mode : chacun sa teinte, actif comme inactif — un mois choisi
   doit se voir d'un coup d'œil au milieu de douze. Teintes CLAIRES : la couleur
   signale la sélection, elle ne l'assombrit pas. Navy et or de la charte. */
.filter-pill.dlb-mois        { width: 56px; color: #46618a; }
.filter-pill.dlb-mois.active { background: #e6edf9; color: #243B5C; box-shadow: inset 0 0 0 1.5px #243B5C; }
.filter-pill.dlb-annee       { width: 62px; color: #8a6a1f; }
.filter-pill.dlb-annee.active{ background: #fdf3e2; color: #8a6a1f; box-shadow: inset 0 0 0 1.5px #D4A047; }
.filter-pill.dlb-mode.active { background: #e4f4f1; color: #0f766e; box-shadow: inset 0 0 0 1.5px #0f766e; }
.filter-pill.dlb-large.active[data-champ="soc"] { background: #e6edf9; color: #243B5C; box-shadow: inset 0 0 0 1.5px #243B5C; }

/* Aperçu ouvert : les filtres s'effacent et le bulletin prend toute la hauteur.
   Le rappel du périmètre reste affiché (ligne de résumé) ; fermer l'aperçu (×)
   les fait revenir. Choisir un mois et lire un bulletin sont deux moments
   différents : ils n'ont pas à se partager l'écran. */
.dlb-visu-on .dlb-filtres { display: none; }
.dlb-visu-on #dlb-resume  { margin-top: 0; padding: 6px 10px; font-size: 12px; }
.dlb-visu-on #dlb-visu-frame { height: 72vh; }   /* + entête, résumé et pied ≈ pleine hauteur */
</style>
<script>
(function(){
  const URL_ZIP     = <?= json_encode(app_url('/api/rh_bulletins_zip.php'), JSON_UNESCAPED_SLASHES) ?>;
  const URL_DECOUPE = <?= json_encode(app_url('/api/rh_bulletins_decoupe.php'), JSON_UNESCAPED_SLASHES) ?>;
  const MOIS = <?= json_encode(array_slice($dlbMoisCourt, 1), JSON_UNESCAPED_UNICODE) ?>;
  // Valeurs de départ : celles de l'écran, pour que le geste le plus fréquent
  // (« ce que je regarde ») soit déjà prêt à valider. mode 0 = par agence.
  const choix = { soc: <?= (int)$dlbChoixSoc ?>, mois: <?= (int)$mois_sel ?>, annee: <?= (int)$annee_sel ?>, mode: 0 };
  ...
  const qs = () => 'soc=' + choix.soc + '&mois=' + choix.mois + '&annee=' + choix.annee;

  /* Changer un critère invalide l'aperçu déjà affiché : classer d'après une
     analyse qui ne correspond plus au périmètre serait une faute lourde. */
  function reinitApercu(){
    el('dlb-zone').style.display = 'none';
    el('dlb-apercu').innerHTML = '';
    // Le composeur porte sur une analyse précise : changer de périmètre le referme.
    el('dlb-mail').style.display = 'none';
    el('dlb-retour').style.display = 'none';
    el('dlb-confirmer').style.display = 'none';
    fermerVisu();
    el('dlb-zip').style.display = 'none';
    el('dlb-classer').style.display = 'none';
    el('dlb-envoyer-tout').style.display = 'none';
    el('dlb-etat').textContent = '';
    el('dlb-valider').style.display = '';
    el('dlb-valider').textContent = choix.mode ? '🔎 Analyser les bulletins' : '📥 Télécharger le ZIP';
    el('dlb-boite').style.maxWidth = '660px';
  }

  window.fermerVisu = function(){
    const v = el('dlb-visu');
    if (!v) return;
    v.style.display = 'none';
    el('dlb-visu-frame').src = 'about:blank';
    el('dlb-zone').style.display = el('dlb-apercu').innerHTML ? 'block' : 'none';
    el('dlb-boite').style.maxWidth = el('dlb-apercu').innerHTML ? '980px' : '660px';
    el('dlb-boite').classList.remove('dlb-visu-on');   // les filtres reviennent
  };

  /* Affiche le PDF QUI PARTIRA : le serveur le régénère à partir des pages qu'il
     a lui-même déterminées. Le navigateur ne transmet aucune liste de pages. */
  window.voirBulletin = function(depotId, idUser, nom){
    el('dlb-zone').style.display = 'flex';
    el('dlb-visu').style.display = 'block';
    el('dlb-boite').style.maxWidth = '1560px';
    el('dlb-boite').classList.add('dlb-visu-on');      // filtres repliés, place au PDF
    el('dlb-visu-titre').querySelector('span').textContent = nom;
    el('dlb-visu-ctl').innerHTML = '<span style="color:#64748b;">Contrôle en cours…</span>';
    /* L'URL doit CHANGER à chaque ouverture. Sans cela, redemander le bulletin
       d'un même salarié réutilise l'iframe telle quelle et le navigateur ressert
       le PDF précédent : on classe une v3 et l'écran reste sur la v2. */
    const url = URL_DECOUPE + '?mode=pdf&depot=' + depotId + '&user=' + idUser + '&' + qs()
              + '&_=' + Date.now();
    /* #toolbar=0 : la barre du lecteur PDF (numéro de page, zoom, impression)
       double celle du navigateur et vole 40 px de hauteur au document. On ne
       garde que la page elle-même, ajustée à la largeur. */
    el('dlb-visu-frame').src = url + '#toolbar=0&navpanes=0&statusbar=0&view=FitH';
    // Le verdict du contrôle voyage dans les en-têtes : on le lit séparément.
    fetch(url, {credentials:'same-origin', method:'HEAD', cache:'no-store'})
      .then(r => {
        const ok = r.headers.get('X-Controle-Ok') === '1';
        const motif = decodeURIComponent(r.headers.get('X-Controle-Motif') || '');
        // Quelle version est RÉELLEMENT affichée : coffre-vN, ou extraction du dépôt.
        const src = r.headers.get('X-Bulletin-Version') || '';
        const badge = src.indexOf('coffre') === 0
          ? '<span style="color:#15803d;font-weight:700;"> · ' + esc(src.replace('coffre-', '')) + ' du coffre</span>'
          : (src ? '<span style="color:#b45309;font-weight:700;"> · découpe du dépôt (pas encore classé)</span>' : '');
        el('dlb-visu-ctl').innerHTML = (ok
          ? '<span style="color:#15803d;font-weight:700;">✅ Contrôle passé</span> <span style="color:#64748b;">— ce PDF ne mentionne que ce salarié.</span>'
          : '<span style="color:#b91c1c;font-weight:700;">⛔ Envoi bloqué</span> <span style="color:#b91c1c;">— ' + esc(motif) + '</span>') + badge;
      })
      .catch(() => { el('dlb-visu-ctl').innerHTML = '<span style="color:#64748b;">Contrôle indisponible.</span>'; });
  };

  /* Envoyer passe TOUJOURS par le composeur : on relit le message et la liste des
     destinataires avant qu'un courrier ne parte au personnel. Aucun envoi direct. */
  let cibleEnvoi = 0;

  window.ouvrirComposeur = function(idUser){
    cibleEnvoi = idUser || 0;
    fermerVisu();
    el('dlb-zone').style.display = 'none';
    el('dlb-mail').style.display = 'block';
    el('dlb-boite').style.maxWidth = '760px';
    el('dlb-zip').style.display = 'none';
    el('dlb-classer').style.display = 'none';
    el('dlb-envoyer-tout').style.display = 'none';
    el('dlb-retour').style.display = '';
    el('dlb-confirmer').style.display = '';
    el('dlb-dest').innerHTML = 'Chargement…';
    el('dlb-etat').textContent = '';

    fetch(URL_DECOUPE + '?mode=destinataires&user=' + cibleEnvoi + '&' + qs(), {credentials:'same-origin'})
      .then(r => r.json())
      .then(d => {
        if (!d.ok) { el('dlb-dest').innerHTML = '<div style="padding:10px;color:#b91c1c;">' + esc(d.error||'Erreur') + '</div>'; return; }
        if (!el('dlb-sujet').value) el('dlb-sujet').value = d.sujet || '';
        if (!el('dlb-corps').value) el('dlb-corps').value = d.corps || '';
        let h = '<table style="width:100%;border-collapse:collapse;font-size:12.5px;">';
        (d.liste||[]).forEach(function(x){
          const manque = !x.email;
          h += '<tr style="border-bottom:1px solid #f1f5f9;' + (manque ? 'background:#fffbeb;' : '') + '">'
             + '<td style="padding:5px 8px;white-space:nowrap;">' + esc(x.nom)
             + (x.presume ? ' <span style="color:#b45309;" title="Comporte une page rattachée par déduction">·</span>' : '')
             + '</td>'
             + '<td style="padding:5px 8px;color:#94a3b8;">' + esc(x.agence) + '</td>'
             + '<td style="padding:5px 8px;">'
             + (manque
                 ? '<input type="email" class="dlb-mail-add" data-user="' + x.id_user + '" placeholder="adresse manquante — à saisir"'
                   + ' style="width:100%;padding:4px 7px;border:1px solid #f59e0b;border-radius:6px;font-size:12px;">'
                 : '<span style="color:#334155;">' + esc(x.email) + '</span>')
             + '</td></tr>';
        });
        h += '</table>';
        el('dlb-dest').innerHTML = h;
        el('dlb-dest-compte').textContent = '(' + (d.liste||[]).length + ')';
        majAlerte(d.sans_mail || 0);
        el('dlb-dest').querySelectorAll('.dlb-mail-add').forEach(function(i){
          i.addEventListener('input', () => majAlerte(manquantes().length));
        });
      })
      .catch(e => { el('dlb-dest').innerHTML = '<div style="padding:10px;color:#b91c1c;">Erreur : ' + esc(e.message) + '</div>'; });
  };

  function manquantes(){
    return Array.from(el('dlb-dest').querySelectorAll('.dlb-mail-add'))
      .filter(i => !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(i.value.trim()));
  }

  function majAlerte(n){
    el('dlb-mail-alerte').innerHTML = n
      ? '<span style="color:#b45309;font-weight:700;">⚠ ' + n + ' salarié(s) sans adresse.</span>'
        + ' <span style="color:#64748b;">Saisissez-la ci-dessus : elle sera enregistrée sur la fiche du salarié. Sans adresse, le bulletin ne sera pas envoyé.</span>'
      : '<span style="color:#15803d;font-weight:700;">✅ Tous les destinataires ont une adresse.</span>';
  }

  window.fermerComposeur = function(){
    el('dlb-mail').style.display = 'none';
    el('dlb-retour').style.display = 'none';
    el('dlb-confirmer').style.display = 'none';
    el('dlb-zone').style.display = 'block';
    el('dlb-boite').style.maxWidth = '980px';
    el('dlb-zip').style.display = '';
    el('dlb-classer').style.display = '';
    el('dlb-envoyer-tout').style.display = '';
  };

  window.confirmerEnvoi = function(){
    const vides = manquantes().length;
    const total = el('dlb-dest').querySelectorAll('tr').length;
    const partants = total - vides;
    if (partants <= 0) { alert('Aucun destinataire ne dispose d’une adresse : rien à envoyer.'); return; }
    if (!el('dlb-sujet').value.trim()) { alert('L’objet est vide.'); return; }

    let q = 'Envoyer ' + partants + ' bulletin(s) ?';
    if (vides) q += '\n\n' + vides + ' salarié(s) sans adresse seront IGNORÉS.';
    q += "\n\nChaque PDF est relu juste avant l'envoi : s'il ne porte pas l'identité de son destinataire, ou s'il en porte une autre, il est bloqué.";
    if (!confirm(q)) return;

    const emails = {};
    el('dlb-dest').querySelectorAll('.dlb-mail-add').forEach(function(i){
      const v = i.value.trim();
      if (/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(v)) emails[i.dataset.user] = v;
    });

    const b = el('dlb-confirmer');
    b.disabled = true; b.textContent = 'Envoi…';
    const fd = new FormData();
    fd.append('mode','send'); fd.append('user', cibleEnvoi); fd.append('csrf_token', jeton());
    fd.append('sujet', el('dlb-sujet').value);
    fd.append('corps', el('dlb-corps').value);
    fd.append('emails', JSON.stringify(emails));
    new URLSearchParams(qs()).forEach((v,k) => fd.append(k,v));

    fetch(URL_DECOUPE, {method:'POST', body:fd, credentials:'same-origin'})
      .then(r => r.json())
      .then(d => {
        b.disabled = false; b.textContent = "✉️ Confirmer l'envoi";
        if (!d.ok) { alert('Erreur : ' + (d.error||'inconnue')); return; }
        let m = d.message;
        if ((d.adresses_ko||[]).length) m += '\n\nAdresses refusées :\n- ' + d.adresses_ko.join('\n- ');
        if ((d.refuses||[]).length) m += '\n\nBLOQUÉS (non envoyés) :\n- ' + d.refuses.join('\n- ');
        if ((d.echecs||[]).length)  m += '\n\nÉchecs techniques :\n- ' + d.echecs.join('\n- ');
        alert(m);
        if (d.envoyes > 0) fermerComposeur();
      })
      .catch(e => { b.disabled = false; b.textContent = "✉️ Confirmer l'envoi"; alert('Erreur : ' + e.message); });
  };

  // Le ✉️ d'une ligne ouvre le même composeur, restreint à ce salarié.
  window.envoyerBulletins = function(idUser){ ouvrirComposeur(idUser); };

  function peindre(){
    modal().querySelectorAll('.filter-pill[data-champ]').forEach(function(b){
      b.classList.toggle('active', String(choix[b.dataset.champ]) === b.dataset.val);
    });
    const bs = modal().querySelector('.filter-pill[data-champ="soc"].active');
    /* Une seule ligne : le rappel du périmètre, puis la règle en quelques mots.
       La place gagnée va à l'aperçu du bulletin — c'est lui qu'on vient lire. */
    el('dlb-resume').innerHTML =
      '<b>' + (bs ? bs.textContent.trim() : '—') + '</b> · <b>'
      + (MOIS[choix.mois-1] || '?') + ' ' + choix.annee + '</b>'
      + '<span style="color:#94a3b8;"> — '
      + (choix.mode ? 'un bulletin par salarié, aperçu avant classement.'
                    : 'un PDF par agence, rassemblés dans un ZIP.')
      + '</span>';
  }

  window.ouvrirTelechargementBulletins = function(){
    // Un ancêtre avec `transform` (layout ToDoBox) piège `position:fixed` : l'overlay
    // se calerait sur la zone de contenu (après la sidebar) et le modal paraîtrait
    // décalé — surtout élargi pour l'aperçu PDF. On le rattache à <body> : fixed
    // redevient relatif à la fenêtre et le centrage flex couvre tout l'écran.
    const m = modal();
    if (m.parentNode !== document.body) document.body.appendChild(m);
    m.style.display = 'flex';
    reinitApercu(); peindre();
  };
  window.fermerTelechargementBulletins = function(){ modal().style.display = 'none'; };

  document.addEventListener('click', function(e){
    const b = e.target.closest('#dlb-modal .filter-pill[data-champ]');
    if (!b) return;
    choix[b.dataset.champ] = parseInt(b.dataset.val, 10);
    reinitApercu(); peindre();
  });

  window.validerTelechargementBulletins = function(){
    if (!choix.mode) {                       // par agence : téléchargement direct
      window.location = URL_ZIP + '?' + qs();
      fermerTelechargementBulletins();
      return;
    }
    analyser();
  };
  ...
  function analyser(){
    const b = el('dlb-valider');
    b.disabled = true; b.textContent = 'Analyse en cours…';
    el('dlb-apercu').style.display = 'block';
    el('dlb-apercu').innerHTML = 'Lecture des PDF…';

    // Idem pour l'analyse : une réponse JSON servie depuis le cache afficherait
    // l'état d'AVANT le dernier classement.
    fetch(URL_DECOUPE + '?mode=preview&' + qs() + '&_=' + Date.now(),
          {credentials:'same-origin', cache:'no-store'})
      .then(r => r.json())
      .then(d => {
        b.disabled = false;
        if (!d.ok) {
          el('dlb-apercu').innerHTML = '<div style="color:#b91c1c;">' + esc(d.error || 'Analyse impossible.') + '</div>';
          b.textContent = '🔎 Analyser les bulletins';
          return;
        }
        el('dlb-apercu').innerHTML = rendre(d);
        el('dlb-zone').style.display = 'block';
        el('dlb-boite').style.maxWidth = '980px';
        ...
        el('dlb-etat').textContent = r.pages_a_verifier
          ? r.pages_a_verifier + ' page(s) non attribuée(s) ne seront pas classées.'
          : 'Attribution complète.';
        majBoutonClasser(r);
      })
      .catch(e => {
        b.disabled = false; b.textContent = '🔎 Analyser les bulletins';
        el('dlb-apercu').innerHTML = '<div style="color:#b91c1c;">Erreur : ' + esc(e.message) + '</div>';
      });
  }

  /* Le bouton dit où on en est : rien n'est plus trompeur qu'un « Classer » actif
     alors que tout est déjà au coffre — on reclique par doute, et on doute encore. */
  function majBoutonClasser(r){
    const b = el('dlb-classer');
    if (!b) return;
    const total = r.bulletins || 0, faits = r.deja_classes || 0;
    const tout  = total > 0 && faits >= total;
    b.textContent = tout ? '✅ Déjà classé'
                         : '📥 À classer dans les dossiers' + (faits ? ' (' + (total - faits) + ' restant(s))' : '');
    b.disabled = tout;
    b.title = tout ? 'Tous ces bulletins sont déjà au coffre RH.'
                   : 'Déposer chaque bulletin au coffre RH du salarié concerné.';
  }

  function rendre(d){
    const r = d.resume || {};
    let h = '<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:10px 12px;margin-bottom:12px;">'
          + '<b>' + (r.bulletins||0) + '</b> bulletin(s) sur <b>' + (r.agences||0) + '</b> agence(s)';
    if (r.pages_a_verifier) h += ' · <span style="color:#b45309;font-weight:700;">' + r.pages_a_verifier + ' page(s) à vérifier</span>';
    if (r.suites_presumees) h += ' · <span style="color:#64748b;">' + r.suites_presumees + ' suite(s) présumée(s)</span>';
    h += '</div>';

    (d.depots||[]).forEach(function(dep){
      h += '<div style="margin-bottom:14px;">'
         + '<div style="font-weight:800;color:#243B5C;margin-bottom:2px;">' + esc(dep.societe) + ' — ' + esc(dep.agence || 'agence ?')
         + ' <span style="font-weight:400;color:#94a3b8;">(' + (dep.pages||0) + ' pages)</span></div>';
      if (dep.methode) h += '<div style="font-size:11px;color:#94a3b8;margin-bottom:4px;">lecture : ' + esc(dep.methode) + '</div>';
      if (dep.alerte)  h += '<div style="color:#b45309;font-size:12px;margin-bottom:5px;">⚠ ' + esc(dep.alerte) + '</div>';
      if (dep.error) { h += '<div style="color:#b91c1c;">⚠ ' + esc(dep.error) + '</div></div>'; return; }
      if (!(dep.salaries||[]).length) { h += '<div style="color:#b45309;">Aucun salarié reconnu.</div></div>'; return; }

      h += '<table style="width:100%;border-collapse:collapse;font-size:12.5px;">';
      dep.salaries.forEach(function(s){
        const nomJs = esc(s.nom).replace(/'/g, "\\'");
        h += '<tr style="border-bottom:1px solid #f1f5f9;">'
           + '<td style="padding:4px 6px;">' + esc(s.nom) + '</td>'
           + '<td style="padding:4px 6px;color:#94a3b8;">' + esc(s.matricule) + '</td>'
           + '<td style="padding:4px 6px;white-space:nowrap;">p. ' + (s.pages||[]).join(', ')
           + (s.presume ? ' <span style="color:#b45309;" title="Page sans référence, rattachée à la précédente">· suite présumée</span>' : '')
           + '</td>'
           + '<td style="padding:4px 6px;white-space:nowrap;font-size:11px;">'
           + (s.classe
                ? '<span style="color:#15803d;" title="Déjà au coffre RH">✅ classé v' + s.classe.version + '</span>'
                  + (s.classe.envoye ? ' <span style="color:#94a3b8;">· envoyé</span>' : '')
                : '<span style="color:#b45309;" title="Pas encore au coffre RH">à classer</span>')
           + '</td>'
           + '<td style="padding:4px 6px;text-align:right;white-space:nowrap;">'
           + '<button type="button" class="v2-btn" style="padding:2px 8px;font-size:11px;"'
           + ' title="Afficher le PDF qui sera envoyé"'
           + " onclick=\"voirBulletin(" + dep.depot_id + "," + s.id_user + ",'" + nomJs + "')\">👁</button> "
           + '<button type="button" class="v2-btn" id="env-' + s.id_user + '" style="padding:2px 8px;font-size:11px;"'
           + ' title="Envoyer ce bulletin à ce salarié"'
           + ' onclick="envoyerBulletins(' + s.id_user + ')">✉️</button>'
           + '</td></tr>';
      });
      h += '</table>';
      if ((dep.non_attribuees||[]).length) {
        h += '<div style="margin-top:6px;color:#b45309;font-size:12px;">⚠ Pages non attribuées (jamais classées) : '
           + dep.non_attribuees.join(', ') + '</div>';
        // Le début de chaque page orpheline : c'est ce qui permet de comprendre
        // POURQUOI elle n'a pas été reconnue, sans ouvrir le PDF.
        if ((dep.extraits||[]).length) {
          h += '<div style="margin-top:4px;border-left:2px solid #fcd34d;padding-left:8px;">';
          dep.extraits.forEach(function(x){
            h += '<div style="font-size:11px;color:#78716c;margin-bottom:2px;">'
               + '<b>p.' + x.page + '</b> · ' + esc(x.texte || '(page sans texte lisible)') + '…</div>';
          });
          h += '</div>';
        }
      }
  ...
  window.telechargerDecoupeZip = function(){ window.location = URL_DECOUPE + '?mode=zip&' + qs(); };

  window.classerDecoupe = function(){
    if (!confirm("Classer ces bulletins au coffre RH de chaque salarié ?\n\nLes pages non attribuées ne seront pas classées.")) return;
    const b = el('dlb-classer');
    b.disabled = true; b.textContent = 'Classement…';
    const fd = new FormData();
    fd.append('mode','apply'); fd.append('csrf_token', jeton());
    new URLSearchParams(qs()).forEach((v,k) => fd.append(k,v));
    fetch(URL_DECOUPE, {method:'POST', body:fd, credentials:'same-origin'})
      .then(r => r.json())
      .then(d => {
        b.disabled = false;
        if (!d.ok) { b.textContent = '📥 À classer dans les dossiers'; alert('Erreur : ' + (d.error||'inconnue')); return; }
        ...
        alert(msg);
        analyser();      // relit l'état : les lignes classées passent en ✅, le bouton suit
      })
      .catch(e => { b.disabled = false; b.textContent = '📥 À classer dans les dossiers'; alert('Erreur : ' + e.message); });
  };

  document.addEventListener('keydown', function(e){
    if (e.key === 'Escape' && modal() && modal().style.display === 'flex') fermerTelechargementBulletins();
  });
})();
</script>
    ...
    <div class="v2-card-body">
        ...
        <?php else: ?>
            ...
            <div class="workflow-actions">
                    ...
                    <div style="background:#fff;border-radius:14px;max-width:<?= $agenceWf > 0 ? '1080px' : '680px' ?>;width:100%;max-height:92vh;overflow-y:auto;padding:0;box-shadow:0 20px 60px rgba(0,0,0,.4);">
                        ...
                        <div style="padding:20px 24px;display:flex;gap:18px;align-items:stretch;">
                          ...
                          <div style="flex:1 1 0;min-width:0;display:flex;flex-direction:column;">
                            ...
                            <iframe src="<?= h($previewPdfUrl) ?>#navpanes=0&toolbar=1&statusbar=0&view=FitH" style="flex:1;width:100%;min-height:60vh;border:1px solid #e5e7eb;border-radius:10px;background:#fff;"></iframe>
                          </div>
                          <?php endif; ?>
                        </div>
                ...
                <?php /* 4. Le bulletin CORRIGÉ d'un salarié — volontairement séparé de la card 3.
                          La card 3 est le jeu complet de l'agence : il fait foi pour le NET, la
                          comparaison et le récap virements. Y glisser un bulletin unique
                          écraserait cette photo comptable (dépôt de référence remplacé, net de
                          l'agence ramené à un seul salarié). Ici rien de tout ça : le PDF est
                          seulement classé au coffre en nouvelle version, pour la ou les
                          personnes qu'il concerne. */ ?>
                <form method="post" action="rh_salaires.php?<?=h($currentQS)?>" enctype="multipart/form-data" class="workflow-step">
                    <h4>4. Bulletins modifiés</h4>
                    <input type="hidden" name="societe_id" value="<?=h($societe_sel)?>">
                    <input type="hidden" name="mois" value="<?=h($mois_sel)?>">
                    <input type="hidden" name="annee" value="<?=h($annee_sel)?>">
                    <input type="hidden" name="csrf_token" value="<?=h(csrf_token())?>">
                    <?php
                    /* Dernier classement de cette agence sur ce mois : la card doit dire
                       ce qu'elle a déjà reçu, comme les trois précédentes. */
                    $corrigeLast = null;
                    try {
                        $qCorr = $pdo->prepare("SELECT w.date_action, w.commentaire,
                                                       TRIM(CONCAT_WS(' ', u.prenom, u.nom)) AS qui
                                                  FROM rh_salaire_workflow_log w
                                             LEFT JOIN users u ON u.id = w.triggered_by
                                                 WHERE w.id_agence = ? AND w.mois_reference = ? AND w.type_action = ?
                                              ORDER BY w.id DESC LIMIT 1");
                        $qCorr->execute([$agenceWf, sprintf('%04d-%02d-01', (int)$annee_sel, (int)$mois_sel), RH_WF_TYPE_CORRIGE]);
                        $corrigeLast = $qCorr->fetch(PDO::FETCH_ASSOC) ?: null;
                    } catch (Throwable $e) { /* la card reste utilisable sans son état */ }
                    ?>
                    <?php if ($corrigeLast): ?>
                    <div style="background:#dcfce7;border:1px solid #86efac;border-radius:8px;padding:8px 12px;margin-bottom:8px;font-size:11px;color:#166534;text-align:left;align-self:stretch;">
                        ✅ Dernier classement le <b><?= h(date('d/m à H:i', strtotime((string)$corrigeLast['date_action']))) ?></b>
                        <?= $corrigeLast['qui'] ? ' par ' . h((string)$corrigeLast['qui']) : '' ?>
                        <?php if (!empty($corrigeLast['commentaire'])): ?>
                            <div style="color:#3f6212;margin-top:2px;"><?= h((string)$corrigeLast['commentaire']) ?></div>
                        <?php endif; ?>
                        <div style="color:#3f6212;margin-top:2px;">Détail dans « Historique des échanges ».</div>
                    </div>
                    <?php else: ?>
                    <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:8px 12px;margin-bottom:8px;font-size:11px;color:#475569;text-align:left;align-self:stretch;">
                        Un ou plusieurs bulletins corrigés, en une fois. Chacun devient la
                        <b>nouvelle version</b> du salarié concerné ; l'ancienne est archivée.
                        <span style="color:#64748b;">Le récap virements et la comparaison ne bougent pas.</span>
                    </div>
                    <?php endif; ?>
                    <div style="margin-top:auto;align-self:stretch;">
            ...
            function ouvrirWfPreview(logId, label) {
                ...
                document.getElementById('wf-preview-iframe').src = 'rh_salaire_workflow_download.php?id=' + logId + '&inline=1#navpanes=0&toolbar=1&statusbar=0&view=FitH';
