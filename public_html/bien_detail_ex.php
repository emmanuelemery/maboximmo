<?php
// ═══════════════════════════════════════════════════════════════════════
// bien_detail_ex.php — ARCHIVE de l'ancienne page V1 (ex bien_detail.php)
// ═══════════════════════════════════════════════════════════════════════
// Cette page contient la version "pro" historique du détail d'un bien
// (~9300 lignes, onglets linéaires Identification / Caractéristiques /
// DPE / Photos / Prix / Annonce / Diffusion / Documents).
//
// Depuis le 2026-04-20, la page PRINCIPALE est bien_detail.php (ex V2,
// carousel de cards). Cette archive est conservée pour :
//   - Consulter la vue détaillée historique (champs ALUR/copro niche)
//   - Rollback d'urgence si un bug bloquant apparaît dans V2
//
// ⚠️ ATTENTION — depuis la migration annonces_photos du 2026-04-20 :
//   Le code de cette page qui appelle AnnoncePhotosManager (ligne ~1350)
//   dépend de l'ANCIEN schéma annonces_photos (url_photo/variante/…) qui
//   est désormais dans annonces_photos_legacy_20260420. Les INSERT/UPDATE
//   photos d'annonce ici planteront. Utiliser bien_detail.php (V2) pour
//   la gestion des photos d'annonce.
// ═══════════════════════════════════════════════════════════════════════
...
<head>
  ...
  <title>Détail du bien — MaBoxImmo</title>
...
<main class="mbi-main">
  ...
  <header class="mbi-topbar">
    ...
    <nav class="topbar-breadcrumb">
      ...
      <span class="sep">›</span>
      <span class="active"><?= $isEditing ? 'Détail du bien' : 'Nouveau bien' ?></span>
    </nav>
  ...
  <div class="ba-tabs-bar" style="position:sticky;top:var(--topbar-h);z-index:45;background:var(--bg);box-shadow:0 4px 10px -8px rgba(0,0,0,.18);padding:0 28px;display:flex;align-items:center;gap:12px;">
    <nav class="ba-tabs" role="tablist" style="flex:1;min-width:0;">
      ...
      <!-- Onglets masqués dans bien_detail.php (Prix, Description, Annonce, Conformité migrés vers annonce_nouvelle.php) -->
      <button type="button" class="ba-tab" data-tab="prix" style="display:none;">💶 Prix</button>
      <button type="button" class="ba-tab" data-tab="description" style="display:none;">✏️ Description</button>
      <button type="button" class="ba-tab" data-tab="annonce" style="display:none;">📡 Annonce</button>
      <button type="button" class="ba-tab" data-tab="conformite" style="display:none;">✅ Conformité</button>
    </nav>
    <div class="ba-tabs-actions" style="display:flex;gap:8px;flex-shrink:0;margin-left:auto;padding:8px 0;">
      <?php if ($isEditing && $editingBienId > 0): ?>
        <!-- Bouton DIFFUSER UNE ANNONCE : redirige vers annonce_nouvelle avec le bien pré-sélectionné -->
        <a href="<?= h(app_url('/annonce_nouvelle.php?id_bien=' . (int)$editingBienId)) ?>"
           class="ph-save-btn"
           style="background:#0ea5e9;color:#fff;border-color:#0ea5e9;text-decoration:none;"
           title="Créer une nouvelle annonce pour ce bien (réglementations automatiques selon le mandat)">
            ...
            <script>
              (function() {
                ...
                const uploader = new DocumentUploader('docs-uploader-container', {
                  ...
                  applyValueFn: window.__baApplyDpeValue || null,
                  onSuccess: (data) => {
                    // Recharge la page pour mettre à jour la liste des documents
