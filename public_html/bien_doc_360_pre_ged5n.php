...
<div class="bd360-wrapper">
    ...
    <div>
        ...
        <form method="POST" action="api/transaction_upload_commit.php" id="bd360Form">
            ...
            <!-- Card 6 : Classement GED (mini-cards cascade) -->
            <div class="f360-card">
                <h3>📁 Classement GED <span class="count">cliquer pour choisir</span></h3>
                <div style="font-size:11.5px;color:#7a766f;margin-bottom:8px;">
                    Mapping auto depuis type_doc = <code><?= bd360_html($effectiveType ?: '—') ?></code> · N1 contexte = <code><?= bd360_html($contexteN1) ?></code>
                </div>

                <div class="bd360-level-title">🧭 N1 — Métier</div>
                <div class="bd360-cards-row" id="bd360N1Row"></div>
                ...
                <div class="bd360-level-title">📑 N3 — Type documentaire (optionnel)</div>
                ...
                <div id="bd360FolderResolved" class="bd360-info-box" style="display:none;margin-top:10px;">
                    ✅ folder_id = <span id="bd360FolderIdLabel"></span> · <span id="bd360FolderPathLabel"></span>
                </div>

                <input type="hidden" name="ged_n1_slug" id="bd360N1Hidden" value="<?= bd360_html($contexteN1) ?>">
                <input type="hidden" name="ged_n2_slug" id="bd360N2Hidden" value="<?= bd360_html((string)$proposed_n2) ?>">
                <input type="hidden" name="ged_n3_slug" id="bd360N3Hidden" value="<?= bd360_html((string)$proposed_n3) ?>">
...
(function() {
    ...
    const N1_ICONS = {
        '00_a_classer_ia':'📥','01_direction':'⚙️','02_referentiel':'📚','03_rh':'👥',
        '04_syndic':'🏢','05_gestion_locative':'🏠','06_transaction':'🤝','07_comptabilite':'💰',
        '08_juridique_contentieux':'⚖️','09_marketing_communication':'📣','10_modeles_documents':'📄',
        '11_mails_communications':'📧','12_archives':'📦','13_fournisseurs':'🚚',
        '98_referentiel_tech':'🔧','99_a_classer_ia':'🚦','99_parametrage_ged':'🛠️','99_systeme':'🖥️',
    };
    function n2Icon(s) { if(!s)return'📁'; if(s.includes('mandat'))return'📋'; if(s.includes('bail'))return'🔑'; if(s.includes('bien'))return'🏠'; if(s.includes('loyer'))return'💶'; if(s.includes('acte'))return'✍️'; if(s.includes('diagn'))return'🩺'; if(s.includes('assur'))return'🛡️'; if(s.includes('sinistr'))return'⚠️'; if(s.includes('sortie'))return'🚪'; if(s.includes('caf'))return'🏛️'; if(s.includes('proprio'))return'👤'; if(s.includes('fiscal'))return'💸'; if(s.includes('acquereur'))return'🤝'; return'📁'; }
    function n3Icon(s) { if(!s)return'📑'; if(s.includes('dpe'))return'⚡'; if(s.includes('erp')||s.includes('ernmt'))return'🌍'; if(s.includes('amiante'))return'🧪'; if(s.includes('plomb'))return'🧱'; if(s.includes('elec'))return'💡'; if(s.includes('gaz'))return'🔥'; if(s.includes('termit'))return'🐛'; if(s.includes('carrez'))return'📐'; if(s.includes('assur'))return'🛡️'; if(s.includes('bail'))return'🔑'; if(s.includes('mandat'))return'📋'; if(s.includes('quittance'))return'🧾'; if(s.includes('caution'))return'🤝'; if(s.includes('edl'))return'📸'; return'📑'; }
    ...
    const folderH = document.getElementById('bd360FolderIdHidden');
    const folderResolv = document.getElementById('bd360FolderResolved');
    const folderIdLabel = document.getElementById('bd360FolderIdLabel');
    const folderPathLbl = document.getElementById('bd360FolderPathLabel');

    function findN1(s) { return GED_TREE.find(n => n.slug === s); }
    function findN2(n, s) { return n ? n.children.find(x => x.slug === s) : null; }
    function findN3(n, s) { return n ? n.children.find(x => x.slug === s) : null; }
    function esc(s) { return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;'); }

    function renderGrid(container, items, hiddenInput, selectedSlug, iconFn, onChange) {
        container.innerHTML = '';
        if (!items || items.length === 0) {
            container.innerHTML = '<div class="bd360-cards-empty">— aucun élément disponible —</div>';
            return;
        }
        items.forEach(it => {
            const btn = document.createElement('button');
            btn.type = 'button';
            btn.className = 'bd360-card-btn' + (it.slug === selectedSlug ? ' is-selected' : '');
            btn.dataset.slug = it.slug;
            const labelClean = (it.name || it.slug).replace(/^\d+[a-z]?\s*-\s*/, '');
            btn.innerHTML = `<span class="bd360-card-icon">${iconFn(it.slug)}</span><span class="bd360-card-label">${esc(labelClean)}</span><span class="bd360-card-code">${esc(it.slug)}</span>`;
    ...
    function refreshFolder() {
        const n1Node = findN1(n1H.value);
        const n2Node = findN2(n1Node, n2H.value);
        const n3Node = findN3(n2Node, n3H.value);
        const fid = n3Node?.id || n2Node?.id || n1Node?.id || 0;
        folderH.value = fid;
        if (fid) {
            folderResolv.style.display = '';
            folderIdLabel.textContent = fid;
            folderPathLbl.textContent = [n1Node?.name, n2Node?.name, n3Node?.name].filter(Boolean).join(' / ');
    ...
    function onN1Click() {
        const n1Node = findN1(n1H.value);
        n2H.value = ''; n3H.value = '';
        renderGrid(n2Row, n1Node ? n1Node.children : [], n2H, '', n2Icon, onN2Click);
        n3Row.innerHTML = '<div class="bd360-cards-empty">— choisir un N2 d\'abord —</div>';
        refreshFolder(); recomputeName();
    }
    function onN2Click() {
        const n1Node = findN1(n1H.value);
        const n2Node = findN2(n1Node, n2H.value);
        n3H.value = '';
        renderGrid(n3Row, n2Node ? n2Node.children : [], n3H, '', n3Icon, onN3Click);
        refreshFolder(); recomputeName();
    }
    function onN3Click() { refreshFolder(); recomputeName(); }

    // Init cascade
    renderGrid(n1Row, GED_TREE, n1H, INITIAL.n1, (s) => N1_ICONS[s] || '📁', onN1Click);
    n1H.value = INITIAL.n1;
    const i1 = findN1(INITIAL.n1);
    if (i1) {
        renderGrid(n2Row, i1.children, n2H, INITIAL.n2, n2Icon, onN2Click);
        n2H.value = INITIAL.n2;
        const i2 = findN2(i1, INITIAL.n2);
        if (i2) {
            renderGrid(n3Row, i2.children, n3H, INITIAL.n3, n3Icon, onN3Click);
            n3H.value = INITIAL.n3;
        }
    }
    refreshFolder();
