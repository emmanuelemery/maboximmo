<?php
// bien_baux_liste.php — Module Bailleur : liste des baux (table bien_baux) + liens.
// Respecte la sélection propriétaire du dashboard (bailleur_props / props[]) + filtre sur la page.
...
if ($isSuperAdmin) {
    $proprietaires = $pdo->query("
        SELECT p.id, COALESCE(NULLIF(TRIM(p.societe),''), NULLIF(TRIM(CONCAT_WS(' ',p.civilite,p.prenom,p.nom)),''), CONCAT('Propriétaire #',p.id)) AS label
        FROM proprietaires p
        WHERE EXISTS (SELECT 1 FROM bien_baux bb JOIN biens b ON b.id=bb.id_bien WHERE b.id_proprietaire=p.id)
        ORDER BY label")->fetchAll(PDO::FETCH_ASSOC);
} else {
    $st = $pdo->prepare("SELECT p.id, COALESCE(up.label,p.societe,CONCAT(p.prenom,' ',p.nom)) AS label
        FROM user_proprietaires up JOIN proprietaires p ON p.id=up.id_proprietaire WHERE up.id_user=? ORDER BY up.ordre,up.label");
    $st->execute([$userId]); $proprietaires = $st->fetchAll(PDO::FETCH_ASSOC);
}
$allPropIds = array_column($proprietaires, 'id');

// Sélection : respecte le dashboard (props[] GET → session bailleur_props), sinon session
$selectedProps = [];
if (isset($_GET['props'])) {
    foreach ((array)$_GET['props'] as $sid) { $sid=(int)$sid; if ($sid>0 && in_array($sid,$allPropIds)) $selectedProps[]=$sid; }
    $_SESSION['bailleur_props'] = $selectedProps;
} elseif (!empty($_SESSION['bailleur_props'])) {
    foreach ($_SESSION['bailleur_props'] as $sid) { $sid=(int)$sid; if (in_array($sid,$allPropIds)) $selectedProps[]=$sid; }
}
$scopeIds = !empty($selectedProps) ? $selectedProps : $allPropIds;

$where = '1=1';
$bind  = [];
if (!empty($scopeIds)) {
    $where = 'b.id_proprietaire IN (' . implode(',', array_map('intval', $scopeIds)) . ')';
} elseif (!$isSuperAdmin) {
    $where = '0=1';
}

// Par défaut on masque les baux résiliés / expirés (historique) — sinon un bien
// apparaît en double (ancien locataire parti + locataire actuel). ?archives=1 = tout voir.
$showArchives = !empty($_GET['archives']);
if (!$showArchives) {
    $where .= " AND bb.statut NOT IN ('resilie','expire')";
}
...
$pageTitle    = 'Baux — Module Bailleur';
$pageSubtitle = 'Ma Box Bailleur · Baux par propriétaire';
...
<form method="get" style="margin:6px 0 14px;display:flex;gap:8px;align-items:center;flex-wrap:wrap">
  <label style="font-size:12px;color:#5a5650;font-weight:700">🏷️ Propriétaire :</label>
  <select name="props[]" onchange="this.form.submit()" style="padding:6px 10px;border:1px solid #d8d3cb;border-radius:8px;font-size:13px">
    <option value="">— Tous mes propriétaires —</option>
    <?php foreach ($proprietaires as $p): ?>
      <option value="<?= (int)$p['id'] ?>" <?= (count($selectedProps)===1 && (int)$selectedProps[0]===(int)$p['id'])?'selected':'' ?>><?= e($p['label']) ?></option>
    <?php endforeach; ?>
  </select>
  <?php if (!empty($selectedProps)): ?>
    <a href="?props=" style="font-size:12px;color:#2c5687">✕ Voir tous</a>
  <?php endif; ?>
  <?php
    // Toggle baux résiliés/expirés (en conservant la sélection propriétaire courante)
    $__qs = $_GET; if ($showArchives) { unset($__qs['archives']); } else { $__qs['archives'] = 1; }
  ?>
  <a href="?<?= e(http_build_query($__qs)) ?>" style="font-size:12px;color:<?= $showArchives ? '#a85858' : '#7a766f' ?>;margin-left:auto">
    <?= $showArchives ? '◉ Masquer les baux résiliés' : '○ Voir les baux résiliés' ?>
  </a>
</form>
...
<div class="bx-stats">
    ...
    <div class="bx-stat"><strong><?= fmt_euro($totalLoyer) ?></strong><span>Loyer base mensuel cumulé</span></div>
...
<?php else: ?>
    <table class="bx-table">
        ...
        ?>
            <tr>
                <td>
                    <a href="<?= h(app_url('/bail_360.php?id=' . (int)$r['bail_id'])) ?>" class="ref" title="Ouvrir la fiche bail 360° (infos + documents)">#<?= (int)$r['bail_id'] ?> ↗</a>
