 ...
 *
 * ⚠️ QUATRE GARDE-FOUS, aucun clic :
 *   1. même source redéposée ET toujours courante → aucune nouvelle version
 *      (`source_sig`). Si elle a été remplacée depuis, au contraire, on la remet en
 *      tête : c'est le seul geste qui répare une correction écrasée ;
 ...
 *   4. une source ANTÉRIEURE à la version en place ne la remplace jamais
 *      (`source_date`) — sans quoi rejouer un vieux dépôt d'agence enterre le
 *      bulletin corrigé reçu depuis.
 *
...
if (!function_exists('rhbc_classer')) {
                          ...
                          string $via = '', ?int $userId = null): array
    {
        ...
        /* ORIGINE — elle décide de tout ce qui suit.
           Un dépôt d'AGENCE (id > 0) est une source automatique, rejouée à chaque
           analyse : le dédoublonnage la protège des versions inutiles.
           Un bulletin CORRIGÉ déposé à la main est un ACTE, pas une proposition.
           Il doit TOUJOURS produire une version, et devenir celle qui part au
           collaborateur. Le dédoublonner revenait à archiver le fichier dans
           l'historique en refusant le classement — deux vérités qui se
           contredisent, et c'est ce qui s'est produit le 11/08 : trois dépôts
           visibles dans l'historique, zéro effet dans le coffre. */
        $origine = ((int)($depot['id'] ?? 0) > 0) ? 'depot' : 'corrige';

        // ── Garde-fou 1 : même source déjà classée — dépôt d'agence UNIQUEMENT ──
        $sig = rhbc_signature($src, $pages);
        if ($origine === 'depot' && ($deja = rhbc_deja_classe($pdo, $idUser, $mois, $annee, $sig))) return $deja;
        ...
        try {
            /* DATE DE LA SOURCE, pas du classement : un dépôt d'agence de juillet
               rejoué en août reste une pièce de juillet. C'est elle qui décide si
               ce fichier a le droit de remplacer la version en place (garde-fou 4).
               Un dépôt unitaire (bulletin corrigé, id=0) n'a pas d'antériorité : il
               arrive maintenant. */
            $srcDate = trim((string)($depot['source_date'] ?? ''));
            if ($srcDate === '' && $origine === 'depot') $srcDate = trim((string)($depot['created_at'] ?? ''));
            if ($srcDate === '') $srcDate = date('Y-m-d H:i:s');
...
if (!function_exists('rhbc_deja_classe')) {
    /**
     * Ce bulletin a-t-il déjà été produit depuis cette même source, ET est-il toujours
     * la version en place ? Retourne le compte-rendu « inchangé », ou null.
     *
     * ⚠️ Le statut est décisif, et c'était le défaut d'origine : la question posée
     * était « cette source a-t-elle déjà servi ? », sans regarder si le bulletin
     * qu'elle a produit était encore courant. Une correction écrasée par un
     * classement postérieur devenait alors impossible à remettre en place — redéposer
     * le bon PDF renvoyait « déjà classé » et ne faisait rien, en silence, autant de
     * fois qu'on essayait. Une version REMPLACÉE doit pouvoir repasser en tête.
     */
    function rhbc_deja_classe(PDO $pdo, int $idUser, int $mois, int $annee, string $sig): ?array
    {
        if ($sig === '') return null;
        try {
            $st = $pdo->prepare("SELECT id, version, statut FROM rh_bulletins
                                  WHERE id_user = ? AND annee = ? AND mois = ? AND source_sig = ?
                               ORDER BY version DESC LIMIT 1");
            $st->execute([$idUser, $annee, $mois, $sig]);
            if ($d = $st->fetch(PDO::FETCH_ASSOC)) {
                // Remplacée depuis : ce n'est PAS un doublon, c'est une réparation.
                if ((string)$d['statut'] !== 'courant') return null;
...
if (!function_exists('rhbc_classer_fichier')) {
     ...
     *
     * @param array $src  sig, depot_id, pages, via, source_date, origine, id_societe,
     *                    id_agence, societe_nom, agence_nom
     * @return array{ok:bool,statut:string,version:int,motif:string,id:int}
     *         statut : cree | inchange | anterieur | refuse | erreur
     */
                                  ...
                                  array $tousSal, array $src = [], ?int $userId = null): array
    {
        ...
        /* Le dédoublonnage ne s'applique QU'aux sources rejouées automatiquement.
           Un bulletin déposé à la main ('corrige') ou repris pièce à pièce
           ('manuel') est un geste délibéré : il crée toujours sa version et devient
           celle qui part au collaborateur. Sinon le fichier finit visible dans
           l'historique et absent du coffre. */
        $origine = (string)($src['origine'] ?? 'depot');
        if (!in_array($origine, ['depot', 'corrige', 'manuel'], true)) $origine = 'depot';
        if ($origine === 'depot' && ($deja = rhbc_deja_classe($pdo, $idUser, $mois, $annee, $sig))) return $deja;
        ...
        try {
            /* ── Garde-fou 4 : une source ANTÉRIEURE ne remplace jamais la version en place ──
               Le numéro de version était calculé en « max + 1 » sans jamais regarder de
               quand datait la pièce. Résultat vécu le 11/08 : le bouton « À classer
               (4 restant(s)) », cliqué pour rattraper quatre collègues, a rejoué le dépôt
               d'agence du 01/07 sur TOUT le monde — et le bulletin d'origine est repassé
               devant la correction reçue le matin même. Depuis, un envoi serait parti avec
               le PDF faux. Un vrai nouveau jeu du comptable, lui, est plus récent : il passe.
               Contrôlé AVANT la relecture du PDF : inutile de travailler pour rien. */
            $precedent = rhbc_courant($pdo, $idUser, $mois, $annee);
            $srcDate   = trim((string)($src['source_date'] ?? ''));
            if ($srcDate === '') $srcDate = date('Y-m-d H:i:s');

            if ($precedent) {
                $datePrec = trim((string)($precedent['source_date'] ?? ''));
                $tSrc = strtotime($srcDate); $tPrec = $datePrec !== '' ? strtotime($datePrec) : false;
                if ($tSrc !== false && $tPrec !== false && $tSrc < $tPrec) {
                    return ['ok' => true, 'statut' => 'anterieur', 'version' => (int)$precedent['version'],
                            'motif' => 'Pièce du ' . date('d/m/Y', $tSrc) . ' antérieure à la version '
                                     . (int)$precedent['version'] . ' en place (' . date('d/m/Y', $tPrec)
                                     . ') — version courante conservée.',
                            'id' => (int)$precedent['id']];
                }
            }
            ...
            $ctx = [
                ...
                'metadata_extra' => [
                    ...
                    'origine' => $origine, 'source_date' => $srcDate,
                ],
            ];
            ...
            $ins = $pdo->prepare(
                "INSERT INTO rh_bulletins
                    (id_user, id_societe, id_agence, mois, annee, version, statut, ged_document_id,
                     file_path, source_depot_id, source_sig, source_date, origine, pages, detecte_via,
                     remplace_id, created_by)
                 VALUES (?,?,?,?,?,?, 'courant', ?,?,?,?,?,?,?,?,?,?)");
            $ins->execute([
                $idUser, $idSoc ?: null, $idAgence ?: null, $mois, $annee, $version, $gedId ?: null,
                $relDurable, ((int)($src['depot_id'] ?? 0)) ?: null, $sig, $srcDate, $origine,
                ($pages ? implode(',', $pages) : null), $via ?: null,
                $precedent ? (int)$precedent['id'] : null, $userId,
            ]);
...
if (!function_exists('rhbc_classer_depot')) {
     ...
     * à traiter.
     *
     * `preserves` liste les salariés que ce dépôt N'A PAS touchés parce qu'ils ont
     * mieux : une version issue d'une pièce plus récente (typiquement un bulletin
     * corrigé). Rattraper les collègues non classés ne doit pas leur coûter leur
     * correction — et l'écran doit le DIRE, sinon le silence passe pour un oubli.
     *
     * @return array{ok:bool,error:?string,crees:int,inchanges:int,refuses:array,
     *               preserves:array,non_attribuees:array,detail:array}
     */
    function rhbc_classer_depot(PDO $pdo, int $depotId, ?int $userId = null): array
    {
        $out = ['ok' => false, 'error' => null, 'crees' => 0, 'inchanges' => 0,
                'refuses' => [], 'preserves' => [], 'non_attribuees' => [], 'detail' => []];
        ...
        try {
            ...
            foreach ($an['par_salarie'] as $ligne) {
                ...
                elseif ($r['statut'] === 'anterieur')  $out['preserves'][] = $ligne['nom'] . ' : ' . $r['motif'];
