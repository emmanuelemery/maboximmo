...
// Contexte Ma Box Net (sous-domaine) : auto-résolution de l'agence locale
// + texte éditorial éditable propre à cette page métier (agence_net_page).
require_once __DIR__ . '/net_context.php';
$net      = net_context($pdo);
$netPage  = null;
if ($net) {
    if ($agence === null) { $agence = $net['agence']; }   // pas besoin de ?agence=slug
    $netPage = net_page($pdo, $net['id_agence'], $slug);  // texte local éditable
}
...
// Le contenu éditorial local peut surcharger les meta (différenciation SEO).
if ($netPage) {
    if (!empty($netPage['meta_title']))       $metaTitle = (string)$netPage['meta_title'];
    if (!empty($netPage['meta_description'])) $metaDesc  = (string)$netPage['meta_description'];
}
