...
if (!function_exists('oc_norm')) {
    ...
    /** Fallback historique si la table de schéma est absente / agence non renseignée. */
    if (!defined('OC_BASE_RIOM')) define('OC_BASE_RIOM', '01_SERVICE_GESTION/05_RIOM GESTION/0001 - NOUVEAUX DOSSIERS ONEDRIVE/00 - PROPRIETAIRES');

    /**
     * Racines OneDrive d'une agence (une agence peut en avoir PLUSIEURS, ex. LYON =
     * propriétaires + diagnostics). Chaque racine :
     *   ['schema_id'=>int, 'label'=>string, 'base'=>string, 'mode'=>'par_proprietaire'|'par_immeuble', 'drive_user'=>?string]
     * Lit agence_onedrive_schema. Fallback RIOM (schema_id 0) si table absente / agence non renseignée.
     */
    function oc_schema_roots_for_agence(PDO $pdo, string $codeAgence): array {
        static $cache = [];
        $key = trim($codeAgence);
        ...
        if ($key !== '') {
            try {
                $st = $pdo->prepare("SELECT id, label, base_path, mode, drive_user FROM agence_onedrive_schema WHERE code_agence=? AND actif=1 ORDER BY id");
                $st->execute([$key]);
                foreach ($st->fetchAll(PDO::FETCH_ASSOC) as $r) {
                    $roots[] = ['schema_id'=>(int)$r['id'], 'label'=>(string)$r['label'],
                                'base'=>trim((string)$r['base_path'], '/'), 'mode'=>(string)$r['mode'],
                                'drive_user'=>($r['drive_user'] !== null && $r['drive_user'] !== '') ? (string)$r['drive_user'] : null];
                }
            } catch (Throwable $e) { /* table absente → fallback */ }
        }
        if (!$roots) {
            $roots[] = ['schema_id'=>0, 'label'=>'proprietaires', 'base'=>OC_BASE_RIOM, 'mode'=>'par_proprietaire', 'drive_user'=>null];
        }
        return $cache[$key] = $roots;
    }

    /** Base OneDrive principale d'une agence (compat : ancien appelant → 1re racine). */
    function oc_base_for_agence(string $codeAgence): string {
        return oc_schema_roots_for_agence($GLOBALS['pdo'], $codeAgence)[0]['base'];
    }

    /** Applique le drive_user du schéma (sinon le drive métier par défaut). */
    function oc_apply_drive(?string $driveUser): void {
        $u = ($driveUser !== null && $driveUser !== '')
            ? $driveUser
            : (defined('GRAPH_ONEDRIVE_USER') ? (string)GRAPH_ONEDRIVE_USER : '');
        graph_set_drive_user($u);
    }
...
if (!function_exists('oc_match_immeuble_folder')) {
    /** Mode par_immeuble (LYON) : meilleur dossier immeuble pour le proprio, sous $base. */
    function oc_match_immeuble_folder(PDO $pdo, int $proprioId, string $base): ?string {
        $st = $pdo->prepare("SELECT DISTINCT i.adresse_1, i.code_postal, i.ville, i.nom_immeuble
                             FROM biens b JOIN immeubles i ON i.id=b.id_immeuble
                             WHERE b.id_proprietaire=?");
        $st->execute([$proprioId]); $imms = $st->fetchAll(PDO::FETCH_ASSOC);
        if (!$imms) return null;
...
if (!function_exists('oc_resolve_proprio')) {
    /**
     * Résout TOUTES les racines OneDrive d'un proprio (agence multi-racines), avec cache.
     * Renvoie :
     *   ['ok'=>true, 'roots'=>[ ['schema_id','label','mode','base','folder','abs','drive_user','source'] ,…],
     *    'base'=>1re_base, 'folder'=>1er_folder, 'abs'=>1er_abs, 'missing'=>[racines non résolues]]
     *   ['ok'=>false, 'error'=>..., 'base'=>1re_base, 'needs_manual'=>true, 'missing'=>[…]]
     * `ok` = au moins une racine résolue. `missing` = racines à compléter (saisie manuelle).
     */
    function oc_resolve_proprio(PDO $pdo, int $proprioId): array {
        ...
        foreach ($rootsDef as $rd) {
            $sid = (int)$rd['schema_id'];
            // 1) Cache (auto ou manuel) → jamais de « introuvable » une fois mémorisé.
            $cached = oc_cache_get($pdo, $proprioId, $sid);
            if ($cached !== null) {
                $resolved[] = $rd + ['abs'=>$cached, 'base'=>dirname($cached), 'folder'=>basename($cached), 'source'=>'cache'];
                continue;
            }
            // 2) Résolution auto selon le mode de la racine.
            oc_apply_drive($rd['drive_user']);
            $folder = ($rd['mode'] === 'par_immeuble')
                ? oc_match_immeuble_folder($pdo, $proprioId, $rd['base'])
                : oc_find_proprio_folder($pdo, $proprioId, $rd['base']);
            if ($folder === null) { $missing[] = $rd; continue; }
            $abs = trim($rd['base'].'/'.$folder, '/');
            oc_cache_set($pdo, $proprioId, $abs, 'auto', 0, $sid);
            $resolved[] = $rd + ['abs'=>$abs, 'folder'=>$folder, 'source'=>'auto'];
        }

        if (!$resolved) {
            $base = $rootsDef[0]['base'] ?? '';
            return ['ok'=>false, 'error'=>'Dossier OneDrive introuvable pour ce propriétaire', 'base'=>$base, 'needs_manual'=>true, 'missing'=>$missing];
        }
        return ['ok'=>true, 'roots'=>$resolved, 'missing'=>$missing,
                'base'=>$resolved[0]['base'], 'folder'=>$resolved[0]['folder'], 'abs'=>$resolved[0]['abs']];
    }
...
if (!function_exists('oc_proprio_folder_url')) {
    ...
    function oc_proprio_folder_url(PDO $pdo, int $proprioId): array {
        $r = oc_resolve_proprio($pdo, $proprioId);
        if (empty($r['ok'])) return ['ok'=>false, 'error'=>$r['error'], 'base'=>$r['base'] ?? null, 'needs_manual'=>!empty($r['needs_manual'])];
        $path    = $r['abs'];
        $folder  = $r['folder'];
        $encoded = implode('/', array_map('rawurlencode', explode('/', $path)));
...
if (!function_exists('oc_scan_proprio')) {
    ...
    function oc_scan_proprio(PDO $pdo, int $proprioId, ?array $rsvOverride = null): array {
        ...
        $base   = $rsv['base'];
        $folder = $rsv['folder'];
        $absRoot = $rsv['abs'];   // chemin absolu complet du dossier proprio (cache ou auto)
        ...
        $seenMandat = [];
        $rootAbs = '';   // racine courante (change à chaque racine résolue)
        $walk = function(string $rel, int $depth) use (&$walk, &$rootAbs, $add, $matchBail, $oneBien, &$seenMandat, $proprioId) {
            if ($depth > 5) return;
            $abs = $rootAbs . ($rel === '' ? '' : "/$rel");
        ...
        // Parcourt CHAQUE racine résolue (LYON = propriétaires + diagnostics). Le drive_user
        // de la racine est posé avant son parcours (racines pouvant être sur des drives distincts).
        $rootsScanned = [];
        foreach ($rsv['roots'] as $rt) {
            oc_apply_drive($rt['drive_user'] ?? null);
            $rootAbs = $rt['abs'];
            $rootsScanned[] = ['label'=>$rt['label'] ?? '', 'folder'=>$rt['folder'] ?? '', 'mode'=>$rt['mode'] ?? ''];
            $walk('', 0);
        }

        return ['ok'=>true, 'proprio'=>$prop, 'folder'=>$folder, 'base'=>$base, 'items'=>$items,
                'roots'=>$rootsScanned, 'missing'=>$rsv['missing'] ?? [],
