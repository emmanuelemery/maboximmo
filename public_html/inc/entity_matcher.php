...
if (!function_exists('em_normalize_name')) {
     ...
     *  norm texte + strip acronyme entre parenthèses + suppression ponctuation.
     *  Ex : "MR & MME SABY"                       → "mr  mme saby"
     *       "SOCIETE IMMOBILIERE DU RHONE (SIR)"  → "societe immobiliere du rhone"
     */
    function em_normalize_name(string $s): string
    {
        $s = em_normalize_text($s);
        // Strip contenu entre parenthèses (acronymes/alias) AVANT virage ponctuation,
        // pour que "SOC X (ALIAS)" matche "SOC X" en exact (et non en partiel).
...
if (!function_exists('em_normalize_for_search')) {
    /** Tolérant pour pré-filtre SQL LIKE : lower + accents + strip parenthèses.
     *  Garde le reste de la ponctuation (&, ', ., …) pour que "MR & MME SABY"
     *  matche en BDD. Mais retire `(XXX)` pour que la version "(SIR)" matche
     *  le tiers existant sans alias entre parenthèses.
     */
