...
if (!function_exists('rhb_depots')) {
    ...
    {
        // Colonnes de cache seulement si la migration 20260811j est passée : le code
        // doit rester déployable avant elle (sinon SELECT en erreur = écran mort).
        $colsCache = rhb_cache_colonnes($pdo) ? ", c.decoupe_sig, c.decoupe_json" : "";
        $sql = "SELECT c.id, c.id_societe, c.id_agence, c.mois, c.annee, c.file_name, c.file_path, c.created_at,
                       s.nom AS societe_nom, a.nom_agence AS agence_nom{$colsCache}
...
if (!function_exists('rhb_signature_analyse')) {
    /**
     * Empreinte d'une analyse : le PDF **et** les fiches salariés qui servent à
     * l'attribution. Corriger un matricule ou un n° de sécu doit invalider le
     * cache — sinon la reconnaissance réparée resterait masquée par l'ancien
     * résultat, et le salarié introuvable le resterait sans qu'on comprenne.
     */
    function rhb_signature_analyse(string $pdfPath, array $salaries): string
    {
        $parts = [];
        foreach ($salaries as $s) {
            $parts[] = (int)$s['id'] . '|' . ($s['matricule'] ?? '') . '|' . ($s['nom'] ?? '')
                     . '|' . ($s['prenom'] ?? '') . '|' . ($s['num_secu'] ?? '');
        }
        sort($parts);
        return hash('sha256', (string)@hash_file('sha256', $pdfPath) . '::' . implode(';', $parts));
    }
}

if (!function_exists('rhb_cache_colonnes')) {
    /** La migration 20260811j est-elle passée ? (le code doit tourner sans elle) */
    function rhb_cache_colonnes(PDO $pdo): bool
    {
        static $ok = null;
        if ($ok !== null) return $ok;
        try {
            $st = $pdo->query("SHOW COLUMNS FROM `rh_salaires_comparaisons` LIKE 'decoupe_sig'");
...
if (!function_exists('rhb_analyse_depot')) {
    /**
     * L'attribution page → salarié d'un dépôt, MÉMORISÉE.
     *
     * Relire les PDF à chaque ouverture de l'écran coûtait plusieurs secondes pour
     * un résultat identique. On conserve donc le résultat, invalidé dès que le
     * fichier ou une fiche salarié change (voir rhb_signature_analyse).
     *
     * @param array $depot ligne rh_salaires_comparaisons + 'chemin'
     */
    function rhb_analyse_depot(PDO $pdo, array $depot, array $salaries): array
    {
        $chemin = (string)($depot['chemin'] ?? '');
        $depotId = (int)($depot['id'] ?? 0);
        if ($chemin === '' || !is_file($chemin)) {
            return ['ok'=>false, 'error'=>'PDF introuvable sur le serveur.', 'pages'=>0,
                    'par_salarie'=>[], 'non_attribuees'=>[], 'detail'=>[], 'methode'=>'', 'diagnostic'=>[], 'alerte'=>null];
        }
        $sig = rhb_signature_analyse($chemin, $salaries);
        $avecCache = $depotId > 0 && rhb_cache_colonnes($pdo);

        if ($avecCache && ($depot['decoupe_sig'] ?? '') === $sig && !empty($depot['decoupe_json'])) {
            $c = json_decode((string)$depot['decoupe_json'], true);
            if (is_array($c) && !empty($c['ok'])) {
                $c['methode'] = trim((string)($c['methode'] ?? '')) . ' · mémorisée';
                return $c;
            }
        }

        $r = rhb_pages_par_salarie($chemin, $salaries);
        if ($avecCache && !empty($r['ok'])) {
            try {
                $pdo->prepare("UPDATE rh_salaires_comparaisons SET decoupe_sig = ?, decoupe_json = ? WHERE id = ?")
                    ->execute([$sig, json_encode($r, JSON_UNESCAPED_UNICODE), $depotId]);
            } catch (Throwable $e) { error_log('[rhb_analyse_depot] cache KO : ' . $e->getMessage()); }
        }
        return $r;
    }
}
        ...
        foreach ($emp as $i => $e) {
            ...
            if ($e['mat'] !== '' && preg_match('/(?<![A-Z0-9])' . preg_quote($e['mat'], '/') . '(?![A-Z0-9])/', $txt)) {
                ...
                /* Étiquette + zéros de tête éventuels : « MATRICULE : 0014 » → 14.
                   DANS LES DEUX SENS — c'est le défaut qui rendait invisibles tous les
                   matricules courts : les PDF de paie sont des tableaux, et la couche
                   texte y restitue souvent la VALEUR AVANT L'ÉTIQUETTE (« 20 MATRICULE »
                   au lieu de « MATRICULE 20 »). Un matricule de moins de 3 caractères
                   n'étant pas cherchable en texte libre, il ne restait plus AUCUNE prise
                   sur le salarié — sauf son nom, le signal le plus fragile.
                   L'écart toléré entre valeur et étiquette exclut les chiffres, donc
                   seul le nombre immédiatement voisin de l'étiquette peut correspondre :
                   une date ou un montant plus loin sur la ligne ne peut pas se glisser. */
                $etq = '(?:MATRICULE|MATRIC|N\s*MATRICULE|IMMATRICULATION)';
                $val = '0*' . preg_quote($e['matEtq'], '/');
                if (preg_match('/' . $etq . '[^A-Z0-9]{0,6}' . $val . '(?![0-9])/', $txt)
                 || preg_match('/(?<![0-9])' . $val . '[^A-Z0-9]{0,6}' . $etq . '/', $txt)) {
                    $parMat[] = $i;
                }
            }
