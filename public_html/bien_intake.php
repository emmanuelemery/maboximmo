...
$sidebarType = 'agency';
require __DIR__ . '/inc/sidebar_agency.php';
?>
...
<style>
  ...
  @media (max-width: 768px) {
    .intake-drop-row { grid-template-columns: 1fr; }
  }
...
<main class="mbi-main">
  <div class="bl-topbar" style="height:56px;display:flex;align-items:center;gap:8px;padding:0 20px;border-bottom:1px solid var(--stroke,#eee);background:var(--card-bg,#fff);position:sticky;top:0;z-index:100;">
    <button type="button" style="width:32px;height:32px;border-radius:8px;border:none;background:var(--bg-subtle,#f5f5f5);cursor:pointer;display:flex;align-items:center;justify-content:center;color:var(--ink-muted,#888);" onclick="history.back()" title="Retour">
      <svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M15 18l-6-6 6-6"/></svg>
    </button>
    <button type="button" style="width:32px;height:32px;border-radius:8px;border:none;background:var(--bg-subtle,#f5f5f5);cursor:pointer;display:flex;align-items:center;justify-content:center;color:var(--ink-muted,#888);" onclick="history.forward()" title="Avancer">
    ...
    <nav style="display:flex;align-items:center;gap:6px;font-size:13px;font-weight:500;color:var(--ink-muted,#888);">
      <a href="<?= h(app_url('/bien_liste.php')) ?>" style="color:var(--ink-muted,#888);text-decoration:none;">Mes biens</a>
      <span style="color:#ccc;">›</span>
      <span style="color:var(--accent,#f7941d);font-weight:600;">Import intelligent</span>
    </nav>
    <div style="flex:1;"></div>
    <div style="width:32px;height:32px;border-radius:50%;background:var(--accent,#f7941d);color:#fff;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;"><?= strtoupper(substr((string)($_SESSION['username'] ?? 'U'), 0, 1)) ?></div>
  </div>
  ...
  <div class="intake-wrap">
    ...
    <div class="intake-drop-row">
      <div class="intake-drop" id="intake-drop">
        <span class="intake-drop-icon">📥</span>
        <div class="intake-drop-title">Glissez vos PDF ici</div>
        <div class="intake-drop-sub">PDF — max 20 Mo — extraction GPT-4o</div>
        <div class="intake-drop-types">
          <span>📋 Mandat</span>
          <span>⚡ DPE</span>
          <span>🔬 Diagnostics</span>
          <span>📐 Carrez / Boutin</span>
          <span>🌍 ERP</span>
        </div>
      ...
      <div class="intake-photos-zone" id="intake-photos-zone">
        ...
        <div class="title">Glissez vos photos ici</div>
        <div class="sub">JPG / PNG / WebP — multi-sélection</div>
...
(function () {
  ...
  async function uploadFile(file) {
    ...
    try {
      ...
      if ((data.proprietaires_candidats && data.proprietaires_candidats.length > 0) || data.proprietaire_suggested) {
        ...
        // Auto-création si pas de match fort et données extraites disponibles
        const propCands = data.proprietaires_candidats || [];
        const bestScore = propCands.length > 0 ? (propCands[0].score || 0) : 0;
        const suggested = data.proprietaire_suggested;
        if (!data.proprietaire_auto_link && bestScore < 80 && suggested && (suggested.nom || suggested.societe)) {
          linkEntity('proprietaire', 'create', null, suggested, null);
        }
      }
  ...
  async function linkEntity(kind, action, targetId, data, source) {
    ...
    try {
      ...
      // Si c'était une déliaison, réafficher tous les candidats
      if (r.unlinked) {
        list.querySelectorAll('.intake-candidate-card').forEach(c => c.style.display = '');
        list.querySelectorAll('.intake-create-new').forEach(b => b.style.display = '');
        refreshState();
        return;
      }
      ...
      // Masquer les autres candidats non liés + le bouton créer
      list.querySelectorAll('.intake-candidate-card:not(.linked)').forEach(c => c.style.display = 'none');
      list.querySelectorAll('.intake-create-new').forEach(b => b.style.display = 'none');
      refreshState();
    } catch (err) {
      alert('Erreur réseau : ' + err.message);
    }
