...
.conflict-option-label { font-size: 13px; font-weight: 600; color: #333; }
.conflict-option-tag {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!dropzone || !fileInput) return; // éléments pas encore prêts
    ...
    function renderResult(data, file) {
        ...
        if (!data.ok) {
            ...
            if (Object.keys(candidates).length > 0) {
                // Stocker les candidats pour la popup
...
async function applyConflicts(candId) {
    ...
    rows.forEach(row => {
        ...
        // On n'envoie que si l'utilisateur choisit "new" OU a modifié la valeur actuelle
