...
$sql = "
    SELECT i.*,
           ...
           (SELECT GROUP_CONCAT(DISTINCT ig.type) FROM immeubles_gestion ig
              WHERE ig.id_immeuble = i.id AND ig.statut = 'actif') AS mandats,
           (SELECT CONCAT(ug.prenom, ' ', ug.nom) FROM immeubles_gestion ig
              JOIN users ug ON ug.id = ig.id_gestionnaire
              WHERE ig.id_immeuble = i.id AND ig.type = 'SYNDIC'
                AND ig.id_gestionnaire IS NOT NULL LIMIT 1) AS syndic_gestionnaire
    FROM immeubles i
...
<!-- ═══ VUE CARDS ═══ -->
<?php
  // Tri alphabétique sur le titre affiché (nom d'immeuble, sinon ville), croissant/décroissant.
  $alphaDir = entity_card_sort_dir();
  $immCards = $immeubles;
  usort($immCards, function($a, $b) use ($alphaDir) {
      $ta = trim((string)($a['nom_immeuble'] ?: $a['ville']));
      $tb = trim((string)($b['nom_immeuble'] ?: $b['ville']));
      $c  = strcasecmp($ta, $tb);
      return $alphaDir === 'desc' ? -$c : $c;
  });
  entity_card_assets();
  echo '<div class="ec-toolbar">' . entity_card_sort_toggle($alphaDir) . '</div>';
?>
<div class="ec-grid">
    <?php foreach ($immCards as $imm):
        $url = 'immeuble_360.php?id=' . (int)$imm['id'];
        $estSyndic = !empty($imm['mandats']) && stripos((string)$imm['mandats'], 'SYNDIC') !== false;
        $nbLots    = (int)$imm['nb_lots'] ?: (int)($imm['nb_biens'] ?? 0);
        $gest      = $imm['syndic_gestionnaire'] ?: $imm['nom_gestionnaire'];
        $chips = [];
        if ($estSyndic) $chips[] = '<span style="color:#2c5687;font-weight:700">🏛️ SYNDIC</span>';
        $chips[] = '<strong>' . $nbLots . '</strong>&nbsp;lots';
        if ($imm['honoraires_ht']) $chips[] = '<strong>' . number_format((float)$imm['honoraires_ht'], 0, ',', ' ') . ' €</strong>';
        entity_card([
            'accent'    => '#4878a6',
            'url'       => $url,
            'ref'       => (string)$imm['reference_immeuble'],
            'title'     => (string)($imm['nom_immeuble'] ?: $imm['ville']),
            'sub'       => $imm['nom_immeuble'] ? ('<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#a8a49e" stroke-width="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg> ' . h($imm['ville'])) : '',
            'badge'     => typeBadge($imm['type_immeuble']),
            'chips'     => $chips,
            'foot_left' => ($gest ? '👤 ' . h($gest) : '') . ' ' . agStatus($imm['date_ag_prochaine']),
        ]);
    endforeach; ?>
</div>
...
function crgShowPreview(data) {
    ...
    (data.immeubles || []).forEach(function(imm) {
        ...
        (imm.lots || []).forEach(function(lot) {
            // Rétro-compat : ancien format (locataire_nom à plat) → tableau locataires
            var locs = lot.locataires;
            if (!locs) {
                locs = lot.locataire_nom ? [{
            ...
            h += '<div style="padding:6px 0;border-bottom:1px solid #f3f4f6;">';
            h += '<div style="font-weight:600;">🔑 Lot ' + esc(lot.numero_lot)
               + ' <span style="color:#888;font-weight:400;">(' + esc(lot.type_bien || '') + ')</span>'
               + ' — <span style="color:' + (actifs > 0 ? '#16a34a' : '#dc2626') + ';">'
               + (actifs > 0 ? actifs + ' actif' + (actifs > 1 ? 's' : '') : 'vacant') + '</span></div>';
            locs.forEach(function(l) {
                var col = l.actif ? '#16a34a' : '#9ca3af';
                h += '<div style="display:flex;justify-content:space-between;padding:2px 0 2px 16px;">';
                h += '<span style="color:' + col + ';">' + (l.actif ? '● ' : '○ ') + esc(l.nom || '')
                   + (l.actif ? '' : ' <em style="color:#9ca3af;">(ancien)</em>') + '</span>';
                h += '<span style="color:#dc2626;font-weight:600;">' + ((l.total_impaye > 0) ? '⚠ ' + fmt(l.total_impaye) + ' impayé' : '') + '</span>';
                h += '</div>';
            });
            h += '</div>';
        });
