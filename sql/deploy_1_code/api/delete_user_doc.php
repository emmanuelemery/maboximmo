...
try {
    ...
    if ($roleId !== 1 && $currentUserId !== $doc['id_user']) {
        $denied = true;
        if ($agenceScope > 0) {
    ...
    $stmt = $pdo->prepare("DELETE FROM salaires_documents WHERE id = ?");
    $stmt->execute([$docId]);

    // Also hide matching profile documents (rh_documents) if present
    try {
