...
// ── Admin-only ──────────────────────────────────────────────
if (empty($_SESSION['super_admin']) && ($_SESSION['role'] ?? '') !== 'admin') {
                ...
                if (!$parsed || !empty($parsed['error'])) {
                    ...
                    foreach ($parsed['immeubles'] ?? [] as $imm) {
                        ...
                        if ($imm_row) {
                            ...
                            $stmt = $pdo->prepare('INSERT INTO immeubles (id_proprietaire, code_crg, nom, adresse, latitude, longitude) VALUES (?,?,?,?,?,?)');
                            $stmt->execute([
                                $id_proprietaire,
                                $code_crg,
                                $nom,
