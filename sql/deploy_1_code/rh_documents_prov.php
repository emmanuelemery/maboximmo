...
} elseif (($roleId === 1 || $agenceScope > 0) && !empty($usersList)) {
    // Auto-select premier de la liste si pas de user_id en GET
    $viewUserId   = (int)$usersList[0]['id'];
    $viewUserName = trim(($usersList[0]['prenom'] ?? '') . ' ' . ($usersList[0]['nom'] ?? ''));
}
...
unset($rubriques['societe']);
$rhdxHiddenTypesForUser = ['contrat'];
foreach ($rubriques as $rubKey => &$rub) {
    $rub['types'] = array_values(array_filter(
        $rub['types'],
        static fn($t) => !in_array($t['key'] ?? '', $rhdxHiddenTypesForUser, true)
    ));
}
unset($rub);
...
// KPIs + alerte docs manquants dans le page-head
$_missingPills = '';
if ($missingCnt > 0) {
    $_missingPills = '<div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-left:12px">'
        . '<span style="font-size:10px;font-weight:700;color:#8a5040">⚠ ' . $missingCnt . ' manquant' . ($missingCnt > 1 ? 's' : '') . ' :</span>';
    foreach ($missingObligatoires as $m) {
        $_missingPills .= '<span style="font-size:10px;background:rgba(138,80,64,0.08);color:#8a5040;border-radius:4px;padding:2px 8px;font-weight:500;white-space:nowrap">' . h($m) . '</span>';
    }
    $_missingPills .= '</div>';
}

$layout_head_kpis = '
<div class="ph-kpi" style="padding:6px 14px"><div class="ph-kpi-val">' . $totalDocs . '</div><div class="ph-kpi-lbl">Documents</div></div>
<div class="ph-kpi" style="padding:6px 14px"><div class="ph-kpi-val" style="color:#8a5040">' . $missingCnt . '</div><div class="ph-kpi-lbl">Manquants</div></div>
' . $_missingPills . '
';
...
<style>
    /* ── Override local : passage de tout en blanc
       (--bg-primary venait de tokens.css en #e4e8f0, bleu-gris) ── */
    :root {
    ...
    /* ── Grille rubriques ── */
    .rubriques-grid { display:grid; grid-template-columns:repeat(2,1fr) 280px; gap:16px; margin-top:25px; }
    /* Layout 2 zones : rubriques (75%) + dropzone (25%) */
    .docs-layout { display:block; }
    .docs-layout .docs-right .rhdx-card { margin-top:0; }
    @media(max-width:1000px) { .docs-layout { display:block; } .rubriques-grid { grid-template-columns:1fr !important; } .rhdx-card { grid-row:auto !important; } }
...
.rhdx-card {
    background: #ffffff;
    border-radius: 16px;
    box-shadow: 4px 4px 12px #d4d7de, -4px -4px 12px #fff;
    padding: 16px;
    margin-bottom: 0;
}
...
<!-- Ouverture du wrapper 2 colonnes -->
<div class="docs-layout">
<!-- Colonne gauche : rubriques (sera fermée après la grille) -->
<div class="docs-left">
...
(function() {
    ...
    function renderResult(data, file) {
        ...
        if (!data.ok) {
            ...
            // Section "Candidates" (champs en attente — déjà remplis, pas écrasés)
            let candidatesHtml = '';
            const candidates = data.candidates || {};
            if (Object.keys(candidates).length > 0) {
                let items = '';
                for (const [col, diff] of Object.entries(candidates)) {
                    items += `<div>⏸ <code>${escapeHtml(col)}</code> : <s>${escapeHtml(String(diff.old))}</s> → ${escapeHtml(String(diff.new))}</div>`;
                }
                candidatesHtml = `
                    <div class="rhdx-candidates">
                        <div class="rhdx-candidates-title">⏸ En attente de confirmation (champs déjà remplis)</div>
                        <div class="rhdx-candidates-list">${items}</div>
                    </div>`;
            }
    ...
    <?php if (false && $missingObligatoires): /* Déplacé dans le page-head */ ?>
    ...
    <?php foreach ($rubriques as $rubKey => $rub):
        ...
        foreach ($rub['types'] as $t) {
            if (!$t['obligatoire']) continue;
            if ($t['confidentiel'] && $roleId !== 1) continue;
            $found = false;
            foreach ($docs as $d) {
    ...
    <div class="rub-card" data-rubrique="<?= h($rubKey) ?>">
      ...
      <div class="rub-card-body">
        ...
        ?>
        <div class="doc-minicard" data-id="<?= (int)$doc['id'] ?>">
          ...
          <div class="doc-minicard-actions">
            <a href="api/rh_doc_serve.php?id=<?= (int)$doc['id'] ?>" target="_blank" class="doc-icon-btn view" title="Voir">
    ...
    <!-- Card Analyse IA intégrée dans la grille (même niveau que les rubriques) -->
    <div class="rhdx-card" style="grid-column:3;grid-row:1/3">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px">
            <div class="rhdx-head-ico" style="width:36px;height:36px;font-size:18px">🔍</div>
            <div>
                <h3 style="font-size:13px;font-weight:700;color:#2f587d;margin:0">Analyse IA</h3>
                <p style="font-size:10px;color:#8a8680;margin:0">Glissez un document pour analyse automatique</p>
            </div>
        </div>

        <div class="rhdx-dropzone" id="rhdxDropzone" style="padding:20px 14px">
            <div class="rhdx-dropzone-ico" style="font-size:28px">📥</div>
            <div class="rhdx-dropzone-txt" style="font-size:11px">Cliquez ou glissez ici</div>
            <div class="rhdx-dropzone-hint" style="font-size:10px">PDF, JPG, PNG · max 10 Mo</div>
