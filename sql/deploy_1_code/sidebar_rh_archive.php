<?php
// sidebar_rh.php — Composant sidebar MaBoxImmo / Module RH
// Inclusion : include 'sidebar_rh.php';

// Détection automatique par nom de fichier (rh_rubrique_action.php)
if (!isset($current_page)) {
    $basename = basename($_SERVER['PHP_SELF'], '.php'); // ex: rh_salaire_detail
    $parts = explode('_', $basename);                   // ['rh', 'salaire', 'detail']
    $rubrique = $parts[1] ?? '';                        // 'salaire'
    // Correspondances rubrique → clé module
    $rubrique_map = [
        'salaire'   => 'salaires',
        'salaires'  => 'salaires',
        'conge'     => 'conges',
...
$rh_modules = [
  'dashboard'   => ['label' => 'Tableau de bord',        'color' => '#5b7eba', 'icon' => 'home',      'url' => 'rh_dashboard.php'],
  'conges'      => ['label' => 'Gestion Congés',         'color' => '#5b7eba', 'icon' => 'users',     'url' => 'rh_conges.php'],
  'salaires'    => ['label' => 'Gestion Salaires',       'color' => '#5b80a8', 'icon' => 'monitor',   'url' => 'rh_salaires.php'],
  'frais'       => ['label' => 'Frais kilométriques',    'color' => '#5b8296', 'icon' => 'briefcase', 'url' => 'rh_indemnite_km.php'],
  'documents'   => ['label' => 'Documents RH',           'color' => '#5b8484', 'icon' => 'file',      'url' => 'rh_documents.php'],
  'entretiens'  => ['label' => 'Entretiens Individuels', 'color' => '#5b8672', 'icon' => 'users-2',   'url' => 'rh_entretien_liste.php'],
  'emails'      => ['label' => 'Emails Équipe',          'color' => '#5b8a60', 'icon' => 'mail',      'url' => 'rh_mails.php'],
];
...
.sb-pill {
  ...
  border-radius: 999px;
  font-family: 'Sora', sans-serif;
  box-shadow: 5px 5px 12px #c8c4be, -5px -5px 12px #f5f2ed;
  text-decoration: none;
}
.sb-pill.active {
  box-shadow: inset 4px 4px 10px var(--shadow-dark), inset -1px -1px 3px rgba(255,255,255,0.35);
  overflow: hidden;
}
...
.sb-pill-ico {
  ...
  flex-shrink: 0;
  background: #ede8e0;
  box-shadow: 3px 3px 6px var(--shadow-dark), -3px -3px 6px #f5f2ed;
}
.sb-pill.active .sb-pill-ico {
  box-shadow: inset 2px 2px 5px var(--shadow-dark), inset -2px -2px 5px #f5f2ed;
}
.sb-lbl {
  font-size: 11.5px;
  font-weight: 500;
...
/* Boutons RH avec trait gauche coloré + fond teinté */
.sb-rh {
  display: flex;
  align-items: center;
  padding: 0 14px 0 0;
...
.sb-mtog {
  ...
  border-radius: 999px;
  background: #ede8e0;
  box-shadow: inset 3px 3px 6px var(--shadow-dark), inset -3px -3px 6px #f5f2ed;
  flex-shrink: 0;
  pointer-events: none;
}
.sb-mthumb {
...
<nav class="sb" aria-label="Navigation principale">
  ...
  <a href="?module=agency" class="sb-pill <?= ($current_page === 'agency') ? 'active' : '' ?>">
    <div class="sb-pill-ico"><?= svg_icon($icons['home'], '#36577d') ?></div>
    <span class="sb-lbl">Agency</span>
  </a>

  <a href="?module=net" class="sb-pill <?= ($current_page === 'net') ? 'active' : '' ?>">
    <div class="sb-pill-ico"><?= svg_icon($icons['globe'], '#7a9060') ?></div>
  ...
  <!-- RESSOURCES HUMAINES -->
  <div class="sb-section">Ressources Humaines</div>

  <?php foreach ($rh_modules as $key => $mod): ?>
  <a href="<?= $mod['url'] ?>"
