...
$taskDocsDone    = $nbDocs >= $requiredDocsCount;  // étape 1
$taskProfilDone  = $profilePct >= 100;              // étape 2
$taskCongesDone  = !empty($user['user_conges_validated_at']); // étape 3
$taskAgenceDone  = !empty($user['user_agence_confirmed_at']); // étape 4

// ─── Ordre séquentiel : chaque étape se débloque après la précédente
// État d'une étape : 'done' | 'active' | 'locked'
$stepDocs   = $taskDocsDone   ? 'done' : 'active';
$stepProfil = $taskProfilDone ? 'done' : ($taskDocsDone   ? 'active' : 'locked');
$stepConges = $taskCongesDone ? 'done' : ($taskProfilDone ? 'active' : 'locked');
$stepAgence = $taskAgenceDone ? 'done' : ($taskCongesDone ? 'active' : 'locked');

// ─── Bascule onboarding_completed si les 4 sont done ───────
$allDone = $taskDocsDone && $taskProfilDone && $taskCongesDone && $taskAgenceDone;
if ($allDone && (int)($user['onboarding_completed'] ?? 0) !== 1) {
...
echo rhdu_render_task(
    '👤',
    '2. Compléter votre profil à 100%',
