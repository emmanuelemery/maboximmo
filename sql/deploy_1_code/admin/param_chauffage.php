...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'toggle_lien_ce') {
        ...
        if ($idTC > 0 && $idEN > 0) {
            $okTC = (int)$pdo->query("SELECT COUNT(*) FROM societe_types_chauffage WHERE id = $idTC AND id_societe = $societeId")->fetchColumn();
            $okEN = (int)$pdo->query("SELECT COUNT(*) FROM societe_energies WHERE id = $idEN AND id_societe = $societeId")->fetchColumn();
            if ($okTC && $okEN) {
                $exists = (int)$pdo->query("SELECT COUNT(*) FROM societe_chauffage_energie WHERE id_societe_type_chauffage = $idTC AND id_societe_energie = $idEN")->fetchColumn();
                if ($exists) {
