...
try {
    ...
    if ($idSociete) {
        $s = $pdo->prepare("SELECT nom FROM societes WHERE id = ? LIMIT 1");
        $s->execute([$idSociete]);
        $societeNom = $s->fetchColumn() ?: '';
    }
...
<div class="main">
  ...
  <div class="top-row">
    ...
    <div class="logo-card">
      <div class="logo-card-label">Agence</div>

      <div class="logo-frame">
        <?php if ($logoAgence && file_exists(__DIR__ . '/' . ltrim($logoAgence, '/'))): ?>
          <img src="<?=e($logoAgence)?>" alt="<?=e($agenceNom)?>">
        <?php else: ?>
          <div class="logo-placeholder-content">
            <div class="logo-placeholder-icon">🏢</div>
            <div class="logo-placeholder-text"><?=e($agenceNom ?: 'Logo à venir')?></div>
          </div>
        <?php endif; ?>
      </div>
